...
compute_ceof <- function(dt, temporal = FALSE, lats.eof = c(-80, -20), n = 1:2) {
   ...
   dt %>% 
      ...
      .[, hgt.cpx := spectral::analyticFunction(hgt),
        by = .(lat, year, lev)] %>% 
      .[, hgt := hgt.cpx] %>%
      EOF(hgt ~ year | lon + lat + lev, n = n, suffix = "cPC", data = .)
}

plot_ceof <- function(data, eof = 1:2, formula = lev  ~  cPC) {
   
   gdata <- data[, cut(ceof[[1]], eof)$right, by = .(dataset, season)]
   
   var <- data[, cut(ceof[[1]], eof)$sdev, by = .(dataset, season)] %>% 
      .[, setNames(paste0(cPC, " (", scales::percent(r2), ")"), 
                   cPC)]
...
```{r eof-espacial_comparacion, fig.cap = "cPC2 calculado usando la anomalía zonal para distintos reanálisis. Parte real en sombreado, parte imaginaria en contornos."}
plot_ceof(ceof[season == "SON"], eof = 2, lev + cPC ~  dataset) 
```
...
ceof[, ceof[[1]]$right, by = .(dataset, season)] %>% 
   ...
   .[season == "SON"] %>% 
   # .[cPC == "cPC2"] %>% 
   .[, FitWave(Re(hgt), 1:3), by = .(lat, lev, dataset, cPC, season)] %>% 
   ...
   facet_nested(lev  ~  cPC, 
                labeller = labeller(lev = AddSuffix(" hPa"))) +
   coord_flip()
```

```{r ceof-temporal-largo, fig.cap = "EOF temporal para ERA20"}
long_ceof$left %>% 
   sep_ReIm(hgt) %>% 
   .[, hgt := scale(hgt), by = .(part, cPC)] %>% 
   # .[, hgt := scale(hgt), by = .(cPC, part, anomaly)] %>% 
   ggplot(aes(year, hgt)) +
   ...
   scale_x_continuous(breaks = AnchorBreaks(1900, 20)) +
   facet_nested(part ~ cPC, labeller = labeller(anomaly = AddPreffix("Anomalía\n"),
                                                part = labs_part))
```
...
linear_ceof$left <- linear_ceof$left %>% 
   sep_ReIm(hgt) %>% 
   .[, hgt := predict(lm(hgt ~ year)), by = .(cPC, part)] %>% 
   dcast(year + cPC ~ part) %>% 
...
trends <- lapply(list(cPC1 = 1, cPC2 = 2, "cPC1+2" = 1:2), predict, object = linear_ceof) %>% 
   rbindlist(idcol = "cPC")
...
plot_reconstruction <- function(pc = "cPC1", binwidth) {
   trends[year %in% range(year)] %>% 
      ...
      scale_y_latitude() +
      scale_linetype_manual(values = c("1" = 1, "-1" = 2), labels = c("1" = "+", "-1" = "-")) +
      facet_nested(lev  ~ cPC + year,
                   labeller = labeller(lev = AddSuffix(" hPa"),
                                       cPC = AddPreffix("Reconstruction with lienar "),
                                       dataset = labs_datasets)) +
      coord_quickmap(ylim = c(NA, -20)) +
      theme(legend.title = element_blank())
}
...
plot_reconstruction("cPC1", 50) 
```

```{r}
plot_reconstruction("cPC2", 5)  
```
...
long_ceof$left %>% 
   sep_ReIm(hgt) %>% 
   .[, hgt := scale(hgt), by = .(part, cPC)] %>% 
   .[, broom::tidy(lm(hgt ~year)), by = .(part, cPC)]
```
...
air_reg <- erai$left %>% 
   ...
   na.omit() %>%
   .[, FitLm(t, hgt, se = TRUE),  
     by = .(lon, lat, cPC, part, lev)] %>% 
   .[term == "hgt"] %>% 
   .[, p.val := Pvaluate(estimate, std.error, df, "fdr"), by = .(cPC, part, lev)]  
...
psi_regr <- ceof[dataset == "ncep"] %>% 
   ...
   na.omit() %>% 
   .[season == "SON"] %>% 
   .[, FitLm(psi.z, hgt = hgt/sd(hgt), se = TRUE), by = .(lat, lon, season, cPC, part, lev)] %>% 
   .[term == "hgt"] %>%
   .[, psi.z := estimate] %>%
   .[, p.val := Pvaluate(estimate, std.error, df, "fdr"), by = .(part, cPC, lev)] %>% 
   .[, c("f.lon", "f.lat") := WaveFlux(.SD), by = .(season, cPC, part, lev)] 
...
sst_regr <- ceof[season == "SON" & dataset == "era"] %>% 
   ...
   na.omit() %>% 
   .[, FitLm(sst, hgt, se = TRUE), by = .(lon, lat, part, cPC)] %>% 
   .[term == "hgt"] %>% 
   .[, p.val := Pvaluate(estimate, std.error, df, "fdr"), by = .(cPC, part)]


plot_reg <- function(reg, variable = "", binwidth = NULL, geom_map = map(lat %between% c(-90, 0))) {
   reg$var <-  variable
...
air_reg %>% 
   ...
   .[part == "R"] %>% 
   .[lat <= 0] %>% 
   plot_reg("Temperature", binwidth = 10) 

hgt_reg %>% 
   .[cPC == "cPC1"] %>% 
   .[part == "R"] %>% 
   plot_reg("Z", binwidth = 500)
...
psi_regr %>% 
   .[cPC == "cPC1"] %>% 
   plot_reg(variable = "Streamfunction", binwidth = 4e5, geom_map = map(lat <= 20)) +
...
datos[season == "SON" & dataset == "era"] %>% 
   ...
   .[, hgt := abs(hgt)] %>% 
   .[, cor(amplitude, hgt), by = .(lat, k, cPC, lev)] %>%
   # .[cPC == "cPC2"] %>%
   ggplot(aes(lat, V1^2)) +
   # geom_zero() +
   ...
   scale_x_latitude() +
   scale_y_continuous("R2 amplitud cPC") +
   scale_color_brewer(palette = "Set1") +
   coord_flip() +
   # facet_nested(cPC + lev ~ k) +
   facet_nested(lev ~ cPC, labeller = labeller(lev = AddSuffix(" hPa")), scales = "free") +
   NULL
```
...
air_reg %>% 
   ...
   .[part == "R"] %>% 
   .[lat <= 0] %>% 
   plot_reg("Temperature", binwidth = 2)  +
   
   hgt_reg %>% 
   .[cPC == "cPC2"] %>% 
   .[part == "R"] %>% 
   plot_reg("Z", binwidth = 100)
```
...
psi_regr %>% 
   .[cPC == "cPC2"] %>% 
   plot_reg(variable = "Streamfunction", binwidth = 1e6, geom_map = map(lat <= 20)) +
   facet_nested(part ~ var,
                labeller = labeller(lev = AddSuffix(" hPa"),
                                    part = labs_part)) +
   geom_vector(aes(dx = f.lon, dy = f.lat,
   ...
   sst_regr %>% 
   .[cPC == "cPC2"] %>% 
   plot_reg(variable = "SST", binwidth = 1, geom_map = map(lat <= 20)) +
   facet_nested(part ~ var,
                labeller = labeller(lev = AddSuffix(" hPa"),
                                    part = labs_part)) 
```
...
sst_regr %>% 
   .[cPC == "cPC2"] %>% 
   plot_reg(variable = "Streamfunction", binwidth = 1, geom_map = map(lat <= 20)) +
   facet_nested(part ~ var,
                labeller = labeller(lev = AddSuffix(" hPa"),
                                    part = labs_part)) 
```
...
```{r, fig.cap = "Composiciones de anomalías de SST para cada tercil del cPC2"}
ceof[season == "SON" & dataset == "era"] %>% 
   .[, ceof[[1]]$left] %>% 
   .[cPC == "cPC2"] %>% 
   sep_ReIm(hgt) %>% 
...
```{r, fig.cap = "Composiciones de anomalías de altura geopotential para cada tercil del cPC2 en 200hPa y 50hPa."}
ceof[season == "SON" & dataset == "era"] %>% 
   .[, ceof[[1]]$left] %>% 
   .[cPC == "cPC2"] %>% 
   sep_ReIm(hgt) %>% 
...
```{r, fig.cap = "Composiciones de anomalías temporales de anomalía zonal de función corriente para cada tercil del cPC2. "}
ceof[season == "SON" & dataset == "era"] %>% 
   .[, ceof[[1]]$left] %>% 
   .[cPC == "cPC2"] %>% 
   sep_ReIm(hgt) %>% 
...
El cPC2 está influenciado por enso, obviamente, pero sólo la parte Real. La parte imaginaria parece insensible al sst o la temperatura tropical. 
...
long_eof$left %>% 
   sep_ReIm(hgt) %>% 
   # .[, hgt := scale(hgt), by = .(cPC, part)] %>% 
   # .[, pw.lin := predict(SiZer::piecewise.linear(year, hgt)$model), by = .(cPC, part)] %>% 
   ggplot(aes(year, hgt)) +
   geom_line() +
   ...
   scale_y_continuous("Hgt") +
   scale_color_brewer(palette = "Set1") +
   facet_nested(part ~ cPC, scales = "free") 
```
...
long_eof$left %>% 
   ...
   scale_y_continuous("Hgt") +
   scale_color_brewer(palette = "Set1") +
   facet_nested(cPC~., scales = "free") 
```



```{r, fig.cap = "Tendencia lineal para los campos de geopotencial reconstruidos a partir de la cPC1, cPC2 y ambas combinadas."}
...
trends <- lapply(list(cPC1 = 1, cPC2 = 2, "cPC1+2" = 1:2), predict, object = long_eof) %>% 
   rbindlist(idcol = "cPC") %>% 
   # .[long_sd, on = "lev"] %>% 
   .[, hgt := Re(hgt)/sqrt(cos(lat*pi/180))] %>% 
   .[, FitLm(hgt, year, se = TRUE), by = .(lon, lat, lev, cPC)] %>% 
   .[term == "year"] %>% 
   .[, p.val := Pvaluate(estimate, std.error, df, "fdr"), by = .(cPC, lev)] %>% 
   rbind(trends_full) %>% 
   .[, cPC := factor(cPC, levels = c("cPC1", "cPC2", "cPC1+2", "Full Data"))]
...
trends %>% 
   ...
   scale_y_latitude() +
   coord_quickmap() +
   facet_nested(lev ~ cPC, labeller = labeller(lev = AddSuffix(" hPa")))

...
ncep.f <- memoise::memoise(function(lat = -90:0, 
   ...
   subset <- list(lat = lat, lon = lon, level = level,
                  time = time)
   ReadNetCDF("DATA/hgt.mon.mean.nc", vars = c(gh = "hgt"),
              subset = subset) %>% 
      setnames(., c("level", "time"), c("lev", "date")) %>% 
      return(.)
}, cache = memoise::cache_filesystem(".rcache"))

file <- ".rcache/ncep.mean.season.Rds"
ncep.mean.season <- cache.file(file, {
   file.ncep <- ".rcache/ncep.vars.Rds"
   ...
   ncep <- cache.file(file.ncep, {
      ...
      ncep <- 
         ncep[, t := ReadNetCDF("DATA/air.mon.mean.nc", out = "vector", 
                                subset = subset)[[1]]] %>% 
         .[, u := ReadNetCDF("DATA/uwnd.mon.mean.nc", out = "vector",
                             subset = subset)[[1]]] %>% 
         .[, v := ReadNetCDF("DATA/vwnd.mon.mean.nc", out = "vector",
                             subset = subset)[[1]]]
      vars <- c("gh", "t", "u", "v")
      vars.z <- paste0(vars, ".z")
      ncep[, c(vars.z) := lapply(.SD, Anomaly), by = .(lat, lev, date), .SDcols = -"lon"]
      ncep[, date := as.Date(date[1]), by = date]
      ncep
   })
   
   ncep.mean.season <- ncep[, lapply(.SD, mean), 
                            by = .(lon, lat, lev, season(month(date)))]
   remove(ncep)
...
ncep.mean.qsseason <- cache.file(file, {
   file.ncep <- ".rcache/ncep.vars.Rds"
   subset <- list(lat = -90:0, 
                  time = lubridate::as_datetime(c("1979-12-01", "2016-02-01")))
   ncep <- cache.file(file.ncep, {
...
```{r}
ncep.f(level = 200)[, gh.minus := gh - BuildField(lon*pi/180, 
...
ggplot(qs.eof$left, aes(lon, lat)) +
   ...
   coord_quickmap() +
   facet_wrap(~PC, labeller = labeller(PC = AddPreffix("PC"))) 
```
...
```{r}
eof.cor <- dcast(qs.eof$right[as.numeric(PC) < 3], date ~ PC, value.var = "gh") %>% 
   setnames(c("1", "2"), c("PC1", "PC2")) %>% 
   .[, date := lubridate::as_datetime(date[1]), by = date] %>% 
   .[ncep.f(level = 200), on = "date"] %>% 
...
plot.fun <- function(plot.PC) {
   ggplot(eof.cor[PC == plot.PC], aes(lon, lat)) +
      geom_contour_fill(aes(z = -value), breaks = AnchorBreaks(0, 0.15, 0)) +
      map.SH +
...
<div class = "notes"> 
Algo parecido para con la correlación con el PC2
</div> 
...
sam[qs.eof$right[as.numeric(PC) < 3], on = "date"] %>% 
   .[, .(cor = cor(sam, gh, use = "complete.obs")), by = .(month(date), PC)] %>% 
   ggplot(aes(month, cor)) +
   geom_col(aes(fill = paste0("PC", PC)), position = "dodge") +
...
eof.cut <- dcast(qs.eof$right, date ~ PC, value.var = "gh") %>% 
   setnames(c("1", "2"), c("PC1", "PC2")) %>% 
   .[, date := lubridate::as_datetime(date[1]), by = date] %>% 
   .[ncep.f(lat = -60:-45), on = "date"] %>% 
   .[, gh.z := Anomaly(gh), by = .(lat, date)] %>% 
   .[, .(PC1 = cor(PC1, gh), PC2 = cor(PC2, gh), 
         PCsum = cor(Mag(PC1, PC2), gh)), 
     by = .(lon, lat, lev, month(date))] %>% 
   melt(id.vars = c("lon", "lat", "lev", "month"), variable.name = "PC")
...
```{r}
plot.fun <- function(plot.PC) {
   ggplot(eof.cut[lat %~% -55 & PC == plot.PC], aes(lon, lev)) +
...
ncep.qs <- ncep.mean.qsseason[, FitWave(gh, k = 3), 
                              by = .(lat, lev, season)] 
...
ggplot(ncep.qs, aes(lat, lev)) +
   ...
   # scale_fill_viridis_c(guide = guide_colorstrip_bottom(10),
   #                      option = "D") +
...
ncep.qs[, stationarity := amplitude/MA]
```


```{r}
knitr::kable(ncep.qs[lat %inrange% region$lat & lev %inrange% region$lev, 
                     .(estacionariedad = mean(stationarity)), by = season])
```
...
plot.levs <- c(200)
binwidth <- 30
ggplot(ncep.mean.qsseason[lev %in% plot.levs], aes(lon, lat)) +
   geom_contour2(aes(z = gh.z, linetype = factor(-sign(..level..))), 
                 breaks = AnchorBreaks(0, binwidth, 0),
                 binwidth = binwidth, color = "black", circular = "x") +
   map.SH +
   geom_label_contour2(aes(z = gh.z), binwidth = binwidth,
                       size = 3) +
   scale_s_map() +
   scale_linetype(guide = "none") +
   facet_wrap( ~ season, ncol = 2, labeller = labeller(lev = lev.lab)) +
   coord_quickmap() 
```
...
ncep.mean.qsseason[lev == 200] %>% 
   copy(.) %>% 
   .[, gh.minus := gh.z - BuildField(lon*pi/180, wave = FitWave(gh.z, k = 1)), 
     by = .(lat, season)] %>% 
   ggplot(aes(lon, lat)) +
   stat_subset(aes(subset = k.full < 3 | !is.finite(k)), color = "gray50",
               fill = "gray50", alpha = 1, geom = "tile") +
   geom_contour2(aes(z = gh.minus, linetype = factor(-sign(..level..))), 
                 color = "black", breaks = AnchorBreaks(0, 10, 0), circular = "x") +
...
ncep.mean.qsseason[lev == 200] %>% 
   copy(.) %>% 
   .[, gh.minus := gh.z - BuildField(lon*pi/180, wave = FitWave(gh.z, k = 1:2)), 
     by = .(lat, season)] %>% 
   ggplot(aes(lon, lat)) +
   stat_subset(aes(subset = k.full < 3 | !is.finite(k)), color = "gray50",
               fill = "gray50", alpha = 1, geom = "tile") +
...
ggplot(stream.mean.season, aes(lon, lat)) +
   stat_contour2(aes(z = psi.z*10^-9, linetype = factor(-sign(..level..))), 
                 color = "black", breaks = AnchorBreaks(0, binwidth, 0),
                 circular = "x") +
...
copy(stream.mean.season) %>% 
   .[, psi.z := psi - BuildField(lon*pi/180, wave = FitWave(psi, k = 0:1)), by = .(lat, season)] %>%

...
qs.eof$right %>% copy() %>% 
   ...
   .[, gh.a := Anomaly(gh), by = .(lon, lat, month(date))] %>% 
   .[!is.na(qs.trim(date)), 
     FitLm(gh.a, PC1, PC2),
     by = .(lon, lat, season = qs.trim(date))] %>% 
   .[regressor != "mean"] -> gh.regr
```


```{r gh-regr, fig.cap = "Regression between PCs and gh."}
ggplot(gh.regr, aes(lon, lat)) +
      ...
      geom_contour_fill(aes(z = estimate), circular = "x",
                        breaks = AnchorBreaks(0, 15, 0)) +
      geom_contour3(aes(z = estimate), circular = "x",
                    breaks = AnchorBreaks(0, 15, 0)) +
      map.SH +
      scale_fill_divergent(breaks = AnchorBreaks(0, 15, 0),
                           guide = guide_colorstrip_bottom()) +
      scale_s_map() +
      coord_quickmap() +
...
qs.eof$right %>% copy() %>% 
   ...
   # .[, date := lubridate::as_datetime(date[1]), by = date] %>%
   .[stream, on = "date"] %>%
   .[, psi := Anomaly(psi), by = .(lon, lat, month(date))] %>% 
   .[!is.na(qs.trim(date)),
     FitLm(psi, PC1, PC2),
    by = .(lon, lat, season = qs.trim(date))] %>% 
...
```{r t-regr, fig.cap = "Regression between 100hPa temperature and PCs"}
binwidth <- 0.15
...
copy(ncep[lev == 200 & lat %between% lats.eof]) %>%
   .[, gh.minus := FilterWave(gh, k = 0:1), by = .(lat, date, lev)] %>% 
   .[, gh := gh.minus*sqrt(cos(lat*pi/180))] %>%
   .[, gh := resid(.lm.fit(cbind(1, date), gh)), by = .(lon, lat)] %>% 
   EOF(data = ., gh ~ lon + lat | date, n = 1:4) -> qs.eof
...
```{r cluster2, fig.cap = 'NO VA. Clústering jerárquico"'}
...
copy(ncep[lev == 200]) %>% 
   ...
   .[, .(full = FitLm(gh, seq_len(.N))$estimate[[2]],
         no.1 = FitLm(gh.1, seq_len(.N))$estimate[[2]]), by = .(lat, lon)] %>% 
   melt(id.vars = c("lat", "lon")) -> gh.slope 
   # .[regressor != "mean"] %>% 
ggplot(gh.slope, aes(lon, lat)) +
   geom_contour_fill(aes(z = value*12), circular = "x",
                     breaks = AnchorBreaks(0, 0.2, 0)) +
   geom_contour2(aes(z = value*12), circular = "x",
                     breaks = AnchorBreaks(0, 0.2, 0))+
   map.SH +
   scale_s_map() +
   scale_fill_divergent() +

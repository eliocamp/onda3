...
compute_eof_ <- function(files, member, time = c("1979-10-01", "2014-12-31"), detrend = FALSE) {
   ...
   if (detrend) {
      ...
      trend <- apply(ensemble_mean, c("lon", "lat", "plev"), function(x) {
         time <- seq_along(x)
         x - .lm.fit(cbind(1, time, time^2), x)$residuals
      }) %>% 
...
trends %>% 
   .[, .(estimate = mean(value)), by = .(model, lon, lat, lev)] %>% 
   .[, interpolate_to_era(.SD, estimate ~ lon + lat), 
     by = .(model, lev)] %>% 
   
   .[, .(estimate = mean(estimate)), by = .(lon, lat, lev)] %>% 
   .[, estimate := Anomaly(estimate), by = .(lat, lev)] %>% 
   ggplot(aes(lon, lat)) +
   geom_contour_fill(aes(z = estimate, fill = stat(level)), 
                     breaks = AnchorBreaks(exclude = 0)) +
   scale_fill_divergent_discretised(guide = guide_coloursteps(barwidth = 20,
                                                              barheight = 1, 
                                                              frame.colour = "black")) +
   geom_qmap(crop = c(ymax = -20)) +
   scale_x_longitude() +

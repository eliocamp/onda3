...
ceofs <- simulaciones %>% 
   .[variable == "zg"] %>%
   .[, .(eof = list(compute_eof(file, member) %>%
                       optimise_rotation())), 
     by = .(experiment, model)]
...
ceofs_detrend <- simulaciones %>% 
   .[variable == "zg"] %>%
   .[, .(eof = list(compute_eof(file, member, detrend = TRUE) %>%
...
# 
# rbind(
#    tendencioso = ceof_era5[, eof[[1]]$left, by = .(model)],
#    destendenciado = ceof_era5_detrend[, eof[[1]]$left, by = .(model)], 
#    idcol = "tendencia") %>% 
#    sep_ReIm(eof) %>% 
#    .[sst_ear5, on = c("time", "model"), allow.cartesian = TRUE] %>% 
#    .[, .(estimate = cor(eof, sst_a)), by = .(lon, lat, cEOF, part)]
...
ceofs_vieja <- simulaciones %>% 
   .[variable == "zg"] %>%
   .[, .(eof = list(compute_eof(file, member, time = c(NA, "1910-12-31")) %>%
                       optimise_rotation())), 
     by = .(experiment, model)]
```
...
# correlation_trends %>% 
#    .[, .SD[which.max(correlation)], by = .(lev, model, cEOF, member)] %>% 
#    .[, angle := NULL] %>% 
#    # .[, .(estimate = mean(correlation)), by = .(lev, model, cEOF)] %>% 
#    ggplot(aes(correlation, model)) +
#    geom_violin(aes(color = cEOF)) +
#    # geom_col(aes(fill = cEOF), position = "dodge") +
#    facet_wrap(~lev) +
...
#    .[, .SD[which.max(correlation)], by = .(lev, model, cEOF, member)] %>% 
#    .[, angle := NULL] %>% 
#    tidyfast::dt_pivot_wider(names_from = cEOF, values_from = correlation) %>%
#    .[, cor(cEOF1, cEOF2), by = .(model, lev)] %>% 
#    .[order(-V1)]
```
...
ceof_largo_p <- simulaciones %>% 
   .[variable == "zg"] %>% 
   .[, eof_proyectado(file, member, model), by = .(experiment, model)] 
```
...
(ref:series-largas-cap) Campos espaciales de cEOF1 en 50 hPa. `r change_plot_links("series-largas", c("Todos los modelos", "Sólo modelos con todos los experimentos"))`

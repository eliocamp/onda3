...
knitr::opts_chunk$set(
   ...
   cache = TRUE, 
   cache.lazy = FALSE,
   cache.extra = paste0(params$cache.extra, 46),
...
```{r sims_damip}
damip_experiments <- c("hist-GHG", "hist-stratO3", "hist-nat")
...
experiments <- c("historical", damip_experiments)
...
era5 <- "/datos/reanalysis/ERA5/mon/era5.mon.mean.nc" %>% 
   ...
   DT(, .(hgt = mean(hgt)), by = .(lon, lat, lev, time = as.integer(year(seasonally(time))))) %>% 
   DT(, model := "ERA5") %>%
   DT(, experiment := "reanalysis") %>% 
   DT(, member := 1L) %>% 
   DT(is.finite(hgt)) %>% 
   DT(, hgt_z := hgt - mean(hgt), by = .(time, lat, lev)) 

set.seed(42)
ceof_era5 <- era5 %>% 
   DT(, hgt_z := hgt_z/sd(hgt_z[time %between% year(climatology)]), by = .(lev)) %>% 
   DT(, hgt_cplx := spectral::analyticFunction(hgt_z), by = .(time, lev, lat)) %>% 
   DT(, eof := hgt_cplx) %>% 
   DT(lat %between% c(-85, -20)) %>%
   DT(, .(eof = list(EOF(formula = eof ~ time + member | lon + lat + lev, n = 1:2, 
                         data = .SD, 
...
ceof_era5_detrend <- era5 %>% 
   copy() %>% 
   DT(, hgt_z := Detrend2(hgt_z), by = .(lon, lat, lev)) %>% 
   DT(, hgt_z := hgt_z/sd(hgt_z[time %between% year(climatology)]), by = .(lev)) %>% 
   DT(, hgt_cplx := spectral::analyticFunction(hgt_z), by = .(time, lev, lat)) %>% 
   DT(, eof := hgt_cplx) %>% 
   DT(lat %between% c(-85, -20)) %>%
...
saveRDS(ref_era5, file.path("cmip", params$season, "ref_era5.Rds"))
```
...
```{r save-ceofs-detrend, cache = FALSE}
saveRDS(ceofs_detrend, file.path("cmip", params$season, "cmip6_ceofs_detrend.Rds"))
```
...
plot_trends <- function(data, which_eof, experiments = "historical") {
...
ceofs_larga <- simulaciones %>% 
   .[variable == "zg"] %>%
   .[, .(eof = list(compute_eof(file, member, time = c(NA_Date_, times[2])) %>%
                       optimise_rotation())), 
     by = .(experiment, model)]
```
...
Fijarse si esto está bien. Estoy calculando una única tendencia por modelo. 
...
eof_proyectado_ <- function(files, member, ..model, time = c(NA_Date_, "2014-12-31")) {
   message("procesando ", files[1])
...
ceof_largo_p <- simulaciones %>% 
   .[variable == "zg"] %>% 
   .[, eof_proyectado(file, member, model, time = c(NA_Date_, times[2])), 
     by = .(experiment, model)] 
```
...
list(
   ...
   ceof_largo_p %>% 
      .[, .SD[all(experiments %in% experiment)], by = model] %>% 
      sep_ReIm() %>% 
      # .[experiment == "historical"] %>% 
...
# ggplot(mapping = aes(estimate, model)) +
#    geom_rect(data = coef_fixed, aes(xmin = conf.low, xmax = conf.high, ymin = -Inf, ymax = Inf), 
#              inherit.aes = FALSE, alpha = .5) +
#    geom_jitter(data = coef_members) +
#    geom_pointrange(data = coef_models, 
#               aes(xmin = conf.low, xmax = conf.high)) +
#    geom_vline(data = coef_fixed, 
#               aes(xintercept = estimate)) +
#    facet_grid(experiment ~ part, scales = "free_y") +
#    coord_cartesian(xlim = 0.02 * c(-1, 1))
...
trend_era <- ceof_era5[, eof[[1]]$left] %>% 
   .[cEOF == "cEOF1"] %>%
   .[, eof := eof/sd(Mod(eof)), by = .(cEOF)] %>% 
   .[time >= 1950] %>% 
   sep_ReIm() %>% 
   .[, FitLm(eof, time = time/10, se = TRUE), 
     by = .(cEOF, part)] %>% 
...
ceof_largo_p %>%
   ...
   geom_hline(data = trend_era, aes(yintercept = estimate)) +
   geom_hline(data = ~.x[, mean(estimate), by = .(model, experiment, part)] %>% 
                 .[, mean(V1), by = .(experiment, part)],
              aes(yintercept = V1, color = experiment), 
              linewidth = 2,
              linetype = 2) +
   
   geom_boxplot(aes(color = experiment), width = 0.5,
                position = position_dodge(.8),
                fill = NA,
                data = ~.x[!startsWith(as.character(model), "ERA5")]) +
   ggforce::geom_sina(aes(color = experiment,
                          group = interaction(part, model),
                          shape = p.value < 0.05)) +
   # scale_color_brewer(NULL, palette = "Set1") +
   scale_color_experiments(guide = "none") +
   scale_x_discrete(NULL) +
   scale_y_continuous("Tendencia") +
   coord_flip()  +
   scale_shape_manual(values = c("TRUE" = 19,

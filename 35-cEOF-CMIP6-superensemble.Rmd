...
```{r ceofs-1-50, fig.width=10, fig.height=10.5, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF1 en 50 hPa."}
plot_ceof(ceofs, 1, 50)
```

```{r ceofs-1-200, fig.width=10, fig.height=10.5, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF1 en 200 hPa."}
...
```{r ceofs-2-50, fig.width=10, fig.height=10.5, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF2 en 50 hPa."}
plot_ceof(ceofs, 2, 50)
```

```{r ceofs-2-200, fig.width=10, fig.height=10.5, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF2 en 200 hPa."}
...
regress_sst_ <- function(..model) {
   ...
   data <- rbindlist(data)
   data[, sst := Detrend(sst), by = .(lon, lat, ensemble)]
   ...
   rotations_cEOF2 <- lapply(angles, function(a) {
      s %>%
         ...
         .[, .(R = cor(Re(hgt2), enso34),
               I = cor(Im(hgt2), enso34)), by = ensemble] %>%
         .[, rotation := a]
   }) %>%
      rbindlist()
   
   best_rotation_cEOF2 <- rotations_cEOF2[I > 0] %>% 
      .[, .SD[which.min(abs(R))]$rotation, by = ensemble] %>% 
      .[, cEOF := "cEOF2"]
   
   regression <- standardised %>% 
      .[model == ..model] %>% 
      .[ensemble %in% unique(data$ensemble)] %>%
      best_rotation_cEOF2[., on = c("ensemble", "cEOF")] %>%
      .[cEOF == "cEOF2", eof := rotate(eof, V1)] %>% 
      .[, V1 := NULL] %>% 
      sep_ReIm(format = "wider") %>% 
      .[, `:=`(Real = Detrend(Real), Imaginary = Detrend(Imaginary)), 
        by = .(model, ensemble, cEOF)] %>% 
...
sst_regression %>% 
   copy() %>% 
   .[, .(estimate = mean(estimate)), by = .(lon, lat, cEOF, model, term)] %>% 
...
for (lev in unique(trends$plev)) {
   trends %>% 
      .[plev == lev] %>% 
      .[, .(estimate = mean(value)), by = .(model, lon, lat, plev)] %>% 
      .[, estimate := Anomaly(estimate), by = .(model, plev, lat)] %>% 
      
      # ggperiodic::periodic(lon = c(0, 360)) %>% 
      ggplot(aes(lon, lat)) +
      geom_contour_fill(aes(z = estimate, fill = stat(level)), 
                        breaks = AnchorBreaks(exclude = 0)) +
      scale_fill_divergent_discretised(paste0(lev/100, " hPa"), 
   ...
   file <- glue::glue("tendencias-{lev/100}.png")
...
ceof_largo_p <- simulaciones %>% 
   .[variable == "zg"] %>% 
   .[, eof_proyectado_(file, member, model), by = model] 
```

...
```{r ceofs-1-50, fig.width=10, fig.height=13, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF1 en 200 hPa."}
...
```{r ceofs-2-50, fig.width=10, fig.height=13, fig.fullwidth = TRUE, fig.cap = "Campos espaciales de cEOF2 en 200 hPa."}
...
eof_proyectado_ <- function(files, member, ..model, time = c(NA, "2014-12-31")) {
   ...
   v <- apply(array, c("time", "ensemble") , function(a) {
      ...
      v <- apply(fa, c("cEOF", "part"), function(f) {
         f %*% a
      }) 
      setNames(c(v) ,
               interaction(rep(dimnames(v)$part, each  = 2), 
                           rep(dimnames(v)$cEOF, 2)))
   })
   names(dimnames(v))[1] <- "part.cEOF"
   
   reshape2::melt(v, value.name = "eof") %>% 
      setDT() %>% 
      .[, c("part", "cEOF") := tstrsplit(part.cEOF, split = ".", fixed = TRUE)] %>% 
      .[, part.cEOF := NULL] %>% 
      .[, `:=`(part = factor(part, levels = c("Real", "Imaginary"), ordered = TRUE),
               cEOF = factor(cEOF, levels = c("cEOF1", "cEOF2"), ordered = TRUE))] %>% 
...
ceofs_larga[, eof[[1]]$left, by = model] %>% 
   ...
   copy() %>% 
   .[, metodo := "ceof"] %>% 
   rbind(copy(ceof_largo_p)[, metodo := "proyectado"]) %>% 
   dcast(model + time + ensemble + cEOF + part ~ metodo, value.var = "eof") %>% 
   .[, cor(proyectado, ceof), by = .(model, cEOF, part)] %>% 
   ggplot(aes(model, V1)) +
...
ceof_largo_p %>% 
   ...
   .[, .(eof = mean(eof)), by = .(model, time, cEOF, part)] %>%
   rbind(., 
         .[, .(eof = mean(eof), model = "MMM"), by = .(time, cEOF, part)]) %>% 
   .[order(time)] %>% 
...
(ref:tendencia-mod-cap) Amplitud promedio de cada modelo (líneas translúcidas) y la amplitud media multimodelo (línea negra) y loess smooth de la media multimodelo (línea azul).

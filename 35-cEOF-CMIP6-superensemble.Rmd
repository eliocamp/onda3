...
list(
   plot_ceof(ceofs, 1, 50) + labs(caption = "Con tendencia"),
   plot_ceof(ceofs_detrend, 1, 50) + labs(caption = "Sin tendencia")
)
```
...
list(
   plot_ceof(ceofs, 1, 200) + labs(caption = "Con tendencia"),
   plot_ceof(ceofs_detrend, 1, 200) + labs(caption = "Sin tendencia")
)
```
...
list(
   plot_ceof(ceofs, 2, 50) + labs(caption = "Con tendencia"),
   plot_ceof(ceofs_detrend, 2, 50) + labs(caption = "Sin tendencia")
)
```
...
list(
   plot_ceof(ceofs, 2, 200) + labs(caption = "Con tendencia"),
   plot_ceof(ceofs_detrend, 2, 200) + labs(caption = "Sin tendencia")
)
```
...
```{r plot_mmm}
plot_mmm <- function(trends = TRUE) {
   if (trends) {
      ceof_cmip6 <- ceofs
      ceof_era <- ceof_era5
      lab <- "Con tendencia"
   } else {
      ceof_cmip6 <- ceofs_detrend
      ceof_era <- ceof_era5_detrend
      lab <- "Sin tendencia"
   }
   
   interpolated <- ceof_cmip6[, eof[[1]]$right, by = model] %>% 
      copy() %>% 
      DT(model != "ERA5") %>% 
   ...
   interpolated %>% 
      ...
      scale_y_latitude(limits = c(-90, NA)) +
      coord_polar() +
      facet_grid(lev ~ cEOF + part, labeller = labeller(lev = AddSuffix(" hPa"),
                                                        cEOF = ceof_labs)) +
      labs(caption = lab)
}
```
...
```{r regress_sst}
standardised <- rbind(
   con = ceofs[, eof[[1]]$left, by = model],
   sin = ceofs_detrend[, eof[[1]]$left, by = model], idcol = "tendencia"
) %>% 
   copy() %>% 
...
trends <- simulaciones %>% 
   .[variable == "zg"] %>% 
   .[, compute_trend(file), by = .(model, member)] %>% 
   .[, lev := plev/100] %>% 
   .[, plev := NULL]
```
...
correlation_trends <- trends %>% 
   copy() %>% 
   .[, value := Anomaly(value), by = .(lat, lev, model, member)] %>% 
   # .[, .(estimate = mean(value)), by = .(model, lon, lat, lev)] %>% 
   .[ceofs[, eof[[1]]$right, by = model][model != "ERA5"], 
     on = c("model", "lon", "lat", "lev"),
     allow.cartesian = TRUE] %>%
   .[, cor_rotations(eof, value, res = 1), by = .(lev, model, cEOF, member)]
```
...
ceof_largo_p %>% 
   sep_ReIm() %>% 
   .[, eof := scale_eof(eof, time <= 1900), by = .(model, part, ensemble, cEOF)] %>%
...
ceof_largo_p %>% 
   copy() %>% 
   .[, eof := scale_eof(eof, time <= 1900), by = .(model, ensemble, cEOF)] %>%
   # .[, eof := Anomaly(eof, time <= 1900), by = .(model, ensemble, cEOF)] %>%
   
   .[, .(eof = mean(Mod(eof))), by = .(model, time, cEOF)] %>% 

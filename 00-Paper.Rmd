...
rbindlist(list(Original = qs.eof$sdev, 
               Rotated = qs.eofr$sdev), fill = TRUE,
          idcol = "rotation") %>% 
...
binwidth <- 0.01
rbindlist(list(Oiginal = CutEOF(qs.eof$right, 1:7),
               Rotated = CutEOF(qs.eofr$right, 1:7)),
          idcol = "rotation") %>%
ggplot(aes(lon, lat)) +
...
qs.eofr$loadings[, FitWave(gh, 2:4), by = .(lat, PC)]  %>%
...
```{r sd-eof, fig.cap = "Standard deviation of each PC for each month"}
qs.eofr$left %>%
   .[, sd(gh), by = .(PC, month(date))] %>% 
   .[qs.eofr$sdev, on = "PC"] %>% 
   .[, V1 := V1*sd] %>% 
...
<!--    scale_x_continuous("Lag (PC1 leads PC2)", -12:12) -->
<!-- ``` -->
...
stream.mean.season <- stream[, .(psi = mean(psi),
                                 psi.z = mean(psi.z)),
                             by = .(lon, lat, lev, season = qs.trim(date))] %>% 
   .[!is.na(season)]
...
ncep[lat %inrange% range(region$lat) &
     ...
     mean(gh), by = .(lat, lon, lev, season = qs.season(date))] %>% 
...
qs.eof$right %>% 
   copy() %>% 
   .[, .(gh.eof = mean(gh)), 
     by = .(PC, season = qs.trim(date), year(date))] %>% 
   .[!is.na(season)] %>% 
   .[, gh.eof := gh.eof/sd(gh.eof), by = .(PC)] -> eof.trim
...
gh.regr <- gh.trim[eof.trim, on = c("season", "year"), 
   ...
   .[, FitLm(gh, gh.eof, se = TRUE), 
                    by = .(lon, lat, season, PC)] %>% 
   .[regressor != "mean"]
```


```{r gh-regr, fig.cap = "(scales) Regression between PCs and gh."}
gh.regr %>% 
   .[, estimate.s := estimate/sd(estimate), by = .(PC)] %>% 
   ggplot( aes(lon, lat)) +
   ...
   scale_s_map() +
   coord_quickmap() +
   facet_grid(PC~season)
```

```{r calc-psi.regr}
stream %>% 
   .[!is.na(qs.trim(date)), 
      .(psi = mean(psi)), by = .(lon, lat, year(date),
                                season = qs.trim(date))] ->
...
stream.trim[eof.trim, on = c("season", "year"), allow.cartesian = TRUE] %>% 
   .[complete.cases(.)] %>% 
   .[, FitLm(psi.a, gh.eof),
     by = .(lon, lat, season, PC)] %>% 
...
psi.regr %>% 
   .[, psi.z := estimate] %>% 
   .[, c("f.lon", "f.lat") := WaveFlux(.SD), by = .(regressor, season)] 
```
```{r}
stream[year(date) == 1998 & month(date) %in% c(1:3), 
       .(psi = mean(psi)), 
       by = .(lon, lat)] %>% 
   ggplot(aes(lon, lat)) +
   geom_raster(aes(fill = psi)) +
   labs(title = "psi media EF")

ggsave("share/psi_EF.png", plot = g)
```
...
```{r calc-ice.regr}
ice[!is.na(concentration) & !is.na(qs.trim(date))] %>% 
   .[, sd := sd(concentration), by = .(x, y, qs.trim(date))] %>% 
   .[sd != 0] %>% 
   .[, FitLm(concentration, PC1, PC2),
     by = .(x, y, season = qs.trim(date))] %>%
...
ice.regr %>% 
   ggplot(aes(x, y)) +
   geom_raster(aes(fill = estimate*100), interpolate = TRUE) +
   geom_contour3(aes(z = estimate*100), breaks = c(-1, 1)) +
   geom_raster(data = ice.mask[x > 14 & concentration > 0], fill = "gray80") +
   geom_contour3(data = ice.mask[x > 14], breaks = 0.99, aes(z = concentration)) +
   scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0),
                        guide = guide_colorstrip_bottom()) +
...
sst <- sst[, sst.a := Anomaly(sst), by = .(lon, lat, month(date))]
```

```{r calc-sst.regr}
sst[!is.na(sst) & !is.na(qs.trim(date)), 
    FitLm(sst.a, PC1, PC2, se = TRUE),
    by = .(lon, lat, season = qs.trim(date))] %>% 
...
oni[, date := lubridate::ymd(paste0(year, "-", month, "-01"))]
...
oni[, broom::tidy(cor.test(gh, ONI)), by = .(PC)] %>% 
   .[, .(PC, estimate, p.value)] %>% 
   knitr::kable(digits = c(1, 2, 9), caption = "Correlation between ONI and each principal component")
```
...
olr[, date := as.Date(date[1]), by = date]
...
olr <- olr[, olr.a := Anomaly(olr), by = .(lon, lat, month(date))]
```
...
setnames(pp, "time", "date")
pp[, date := as.Date(date[1]), by = date]

qs.eof$right %>%
   .[, gh := gh/sd(gh), by = .(PC)] %>% 
   dcast(date ~ PC, value.var = "gh") %>%
   .[pp, on = "date"] %>%
   .[!is.na(PC1) & !is.na(qs.trim(date))] -> pp

pp <- pp[, pp.a := Anomaly(precip), by = .(lon, lat, month(date))]
# pp[, pp.p := precip/mean(precip), by = .(lon, lat, month(date))]
```

```{r calc-pp.regr}
pp[!is.na(qs.trim(date)), 
...
setnames(t, "time", "date")
t[, date := as.Date(date[1]), by = date]
...
qs.eof$right %>%
   ...
   dcast(date ~ PC, value.var = "gh") %>%
   .[t, on = "date"] %>%
   .[!is.na(PC1) & !is.na(qs.trim(date))] -> t

t <- t[, air.a := Anomaly(air), by = .(lon, lat, month(date))]
```

```{r calc-t.regr}
t[!is.na(qs.trim(date)), 
...
copy(ncep[lev == 200 & lat %between% lats.eof]) %>%
   ...
   EOF(data = ., gh ~ lon + lat | date, n = 1:20, B = 100) -> qs.eof2
...
qs.eof$right[PC %in% c("PC1", "PC2"), .(value = mean(gh)), by = .(PC, month(date))] %>%
   dcast(... ~ PC) %>% 
   .[, -1] %>% 
   dist() %>%
   hclust() %>% 
...
copy(ncep[lev == 200]) %>% 
   .[, gh.1 := FilterWave(gh, 0:1), by = .(lat, lev, date)] %>%
   .[, FitLm(gh.1, seq_len(.N), se = TRUE), by = .(lat, lon, month(date))] %>% 
   .[regressor != "mean"] %>%
   ggplot(aes(lon, lat)) +
   geom_contour_fill(aes(z = estimate), circular = "x",
...
```{r}
ncep[lat == -45 & lev == 200] %>% 
   # .[, gh := Anomaly(gh), by = .(lon, lat, month(date))] %>%
   .[, gh.1 := FilterWave(gh, 0:1), by = .(lat, date)] %>%
   .[lon == 290] %>% 
   ggplot(aes(date, gh.1)) + 
   geom_line() + 

...
ggplot(psi.regr, aes(lon, lat)) +
   ...
   geom_streamline(aes(dx = f.lon, dy = f.lat), 
                   skip = 3, circular = "x", L = 10, size = 0.1, 
...
```{r ice-regr, fig.cap = "Regression of standarized PC with antarctic sea ice concentrations (shaded) and regression of standarized PC with 200hPa geopotential height (contours)."}
binwidth <- 10

ggplot(ice.regr[lat < -55], aes(lon, lat)) +
   geom_point(aes(color = estimate*100), size = 0.05) +
   geom_contour3(data = slp.regr, aes(z = estimate),
                 breaks = AnchorBreaks(0, 1, 0), circular = "x") +
   map.SH +
...
```{r qs3-eof, fig.cap = "Relationship between 2 first EOFs and amplitude of wave 3. a) Monthly mean wave 3 amplitude as a function of PC1 and PC2; contours are a representation of the linear fit on the right. b) Amplitude of monthly mean wave 3 amplitude as a function of PC1 and PC2 magnitude."}

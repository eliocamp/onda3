...
ice.drift.regr <- ice.drift.sem %>% 
   ...
   .[N > 10] %>% 
   .[, FitLm(value/10, PC1, PC2, se = TRUE), 
...
```{r}
ice.drift.regr %>% 
   dcast(x + y + season + regressor ~ variable, value.var = "estimate") %>% 
   icemap[., on = c("x", "y")] %>% 
   .[complete.cases(.)] %>% 
   # saveRDS("~/data.Rds")
ggplot(aes(x, -y)) +
    geom_streamline(aes(dx = u*cos(lon*pi/180) + v*sin(lon*pi/180),
                        dy = -u*sin(lon*pi/180) + v*cos(lon*pi/180),
                        color = Mag(..dx.., ..dy..)),
                    skip = 5, arrow.length = 0.1) +

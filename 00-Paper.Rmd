...
```
```{r}
stream[year(time) == 1998 & month(time) %in% c(1:3), 
       .(psi = mean(psi)), 
       by = .(lon, lat)] %>% 
   ggplot(aes(lon, lat)) +
...
```{r calc-ice.regr}
ice[!is.na(concentration) & !is.na(qs.trim(time))] %>% 
   .[, sd := sd(concentration), by = .(x, y, qs.trim(time))] %>% 
   .[sd != 0] %>% 
   .[, FitLm(concentration, PC1, PC2),
     by = .(x, y, season = qs.trim(time))] %>%
...
sst <- sst[, sst.a := Anomaly(sst), by = .(lon, lat, month(time))]
```

```{r calc-sst.regr}
sst[!is.na(sst) & !is.na(qs.trim(time)), 

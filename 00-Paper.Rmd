...
# update_geom_font_defaults(family = "Roboto Condensed")

update_geom_defaults("contour", list(color = "black"))
update_stat_defaults("contour", aes(linetype = factor(-sign(..level..))))
...
ncep[, gh.minus := FilterWave(gh, k = 0:1), by = .(lat, date, lev)]
```
...
region <- expand.grid(lat = c(-65, -40),
                      lev = c(100, 700))
lats.eof <- c(-80, -30)
copy(ncep[lev == 200 & lat %between% lats.eof & !is.na(gh.minus)]) %>%
...
qs.eof$right %>% 
   ...
   scale_x_continuous("Lag (PC1 leads PC2)", 0:12)
```


```{r PCs-anual, fig.cap = "Anual cicle of PC1 and PC2... boxplot represents.... "}
qs.eof$right %>% 
   ggplot(aes(factor(month(date)), gh, group = interaction( PC, month(date)))) +
   geom_boxplot(aes(fill = PC), outlier.alpha = 0) +
...
ncep.mean.season[, gh.z := Anomaly(gh), by = .(lat, lev, season)]
ncep.mean.season[, gh.1 := FilterWave(gh, 0:1), by = .(lat, lev, season)] 
ncep.mean.season[, gh.12 := FilterWave(gh, 0:2), by = .(lat, lev, season)] 
...
binwidth <- 0.002
ggplot(stream.mean.season[lat <= -30], aes(lon, lat)) +
   ...
   #                     alpha = ..step..), skip = 3, circular = "x", L = 15,
   #                 arrow = NULL) +
   geom_streamline(aes(dx = f.lon, dy = f.lat), 
                   skip = 3, circular = "x", L = 10, size = 0.3, 
                   min.dist = 10, arrow.length = 0.2) +
   # geom_vector(aes(dx = f.lon, dy = f.lat), skip = 4, min.mag = 0.02,
   # scale = 150, size = 0.5, arrow.angle = 8, arrow.length = 0.3) +
   geom_map2(data.world[lat < 30], color = "gray20") +
   scale_x_longitude() +
   scale_y_latitude(limits = c(-90, -30)) +
   scale_size(range = c(0, 1)) +
   metR:::scale_mag(1, 0.1) +
   scale_fill_divergent(breaks = MakeBreaks(binwidth, 0),
                        guide = guide_colorstrip_bottom()) +
   facet_wrap(~season, ncol = 3) +
   coord_quickmap()
```
...
stationarity <- ncep %>%
   .[, FitWave(gh, k = 3),
     by = .(lat, lev, date)] %>%
   .[, .(MA = mean(amplitude)), by = .(lat, lev, season = qs.trim(date))] %>%
...
```{r calc-gh.cor}
gh.cor <- dcast(qs.eof$right, date ~ PC, value.var = "gh") %>%
   ...
   melt(id.vars = c("lon", "lat", "lev", "season"), variable.name = "PC")
```


```{r gh-cor1}
plot.fun <- function(plot.PC) {
   ggplot(gh.cor[PC == plot.PC], aes(lon, lat)) +
...
ice.regr %>% 
   .[, value_ := floor(value*100/binwidth)*binwidth] %>% 
   ggplot(aes(x, y)) +
   # geom_raster(aes(fill = value_)) +
   geom_raster(aes(fill = value*100)) +
   # geom_contour3(aes(z = value*100), breaks = AnchorBreaks(0, binwidth, 0)) +
...
ggplot(sst.regr, aes(lon, lat)) +
   ...
   # geom_contour3(aes(z = estimate), breaks = c(-0.2, 0.2)) +

...
ggplot(stream.mean.season, aes(lon, lat)) +
   ...
   geom_streamline(aes(dx = f.lon, dy = f.lat), 
                   skip = 3, circular = "x", L = 20, size = 0.4, 
...
#    geom_contour_fill(aes(z = psi.z), breaks = AnchorBreaks(0, 0.1, 0)) +
#    geom_streamline(aes(dx = f.lon, dy = f.lat), skip = 3, circular = "x",
#                    L = 10, min.dist = 5, arrow.length = 0.2, size = 0.4) +
#    scale_fill_divergent() +
#    map.SH +
#    scale_s_map() +
#    coord_quickmap() +

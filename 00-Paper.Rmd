...
model <- lm(amplitude ~ I(sqrt(PC1^2 + PC2^2)) - 1, data = qs3)
grid <- setDT(expand.grid(PC1 = seq_range(qs3$PC1, 64),
                          PC2 = seq_range(qs3$PC2, 64)))
grid[, amplitude := predict(model, grid)]
smooth <-  qs3[, Smooth2D(amplitude ~ PC1 + PC2, theta = 0.025,
                          x.out = 64*2, y.out = 64*2)]
min <- smooth[which.min(amplitude)] 

ggplot(qs3, aes(PC1, PC2)) +
   geom_cross(size = 0.2) +
   geom_contour(data = grid, aes(z = amplitude, color = ..level..)) +
   geom_point(aes(color = amplitude)) +
   ...
   ggplot(qs3, aes(sqrt(PC2^2 + PC1^2), amplitude)) +
...
ggplot(qs3, aes(phase*180/pi*3, 
                ConvertLongitude(atan2(PC2, PC1)*180/pi)+47)) +
   geom_point() +
   geom_abline() +

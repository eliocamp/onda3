...
ggplot(ncep.qs[lat <= 0], aes(lat, lev)) +
   ...
   geom_contour3(aes(z = amplitude), 
                     breaks = breaks) +
    geom_label_contour2(aes(z = amplitude), 
                     breaks = breaks) +
   surface +
...
ice.regr %>% 
   ...
   geom_raster(data = ice.mask[x > 14], fill = "gray80") +
   scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0),
                        guide = guide_colorstrip_bottom()) +
...
ggplot(olr.regr, aes(lon, lat)) +
   ...
   stat_subset(aes(subset =  abs(estimate) > se*1.6), geom = "point",
               alpha = 0.5, color = "black", size = 0.05,
               data = olr.regr[lon %in% JumpBy(unique(lon), 2) &

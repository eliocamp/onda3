...
ggplot(qs3, aes(PC1, PC2)) +
   ...
   ggplot(qs3, aes(abs(PC2) + abs(PC1), amplitude)) +
...
ggplot(qs3, aes(PC1, PC2)) +
   geom_cross(size = 0.2) +
   # geom_contour(data = grid, aes(z = amplitude, color = ..level..)) +
   # geom_point(aes(color = cos(phase))) +
   stat_summary_hex(aes(z = phase*180/pi), bins = 10) +
   scale_fill_gradientn(colours = c("#DAE136", "#414485", "#DAE136"), 
                        limits = c(0, 120)) +

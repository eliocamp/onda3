...
events %>%
   ...
   scale_fill_divergent(breaks = AnchorBreaks(0, 20, 0),
                        guide = guide_colorstrip_bottom()) -> g
```
...
events %>% 
   ...
   # dcast(month + PC ~ event, value.var = "N") %>% 
   ggplot(aes(month, N)) +
...
g + facet_grid_paginate(month ~ PC + variable, ncol = 4, 
                        nrow = 6, page = 1,
              labeller = labeller(month = month.abb))
```

```{r comp-events-1, fig.cap = "Mean gh anomaly (positive - neutral; negative - neutral) for positive and negatives events for each PC and each month. (continued)"}
g + facet_grid_paginate(month ~ PC + variable, ncol = 4, 
                        nrow = 6, page = 2,
              labeller = labeller(month = month.abb))
```
...
ncep[lev == 200, .(gh = mean(gh)), 
     by = .(lon, lat, year(time), 
            season = qs.trim(time))] %>% 
   .[!is.na(season)] %>% 
   .[, gh := Detrend(gh), by = .(lon, lat, season)] -> gh.trim 
...
stream %>% 
   .[!is.na(qs.trim(time)), 
     .(psi = mean(psi)), by = .(lon, lat, year(time),
                                season = qs.trim(time))] %>% 
   .[, psi.a := Detrend(psi), by = .(lon, lat, season)] -> stream.trim
...
ice.eof <- ice[!is.na(qs.trim(time)), 
               EOF(concentration ~ x + y | time, n = 1:2)$left,
               by = .(season = qs.trim(time))]

ice.eof_time <- ice[!is.na(qs.trim(time)), 
...
remove(ice)
ice.trim[, concentration.a := Detrend(concentration), 
         by = .(lon, lat, season)]
```
...
slp.trim <- slp[!is.na(qs.trim(time)), 
                .(slp = mean(slp)), 
                by = .(lon, lat, year(time), season = qs.trim(time))]

slp.trim[, slp.a := Detrend(slp), by = .(lon, lat, season)]

eof.trim %>% 
   slp.trim[., on = c("season", "year")] %>% 
   .[complete.cases(.)] %>% 
...
```{r sam.trim, fig.cap = "Relationship between PC1 and PC2 and SAM."}
sam <- fread("DATA/sam.monthly.txt")
colnames(sam) <- c("year", "month", "sam")
sam.trim <- sam[!is.na(qs.trim(month)), 
                .(sam = mean(sam)), 
                by = .(year, season = qs.trim(month))]

eof.trim %>% 
   melt(id.vars = c("year", "season"), variable.name = "PC") %>%
   sam.trim[., on = c("season", "year")] %>% 
...
p1 <- ggplot(qs3, aes(PC1, PC2)) +
   ...
   scale_color_viridis_c(limits = c(0, 120),
                         oob = scales::squish) +
...
```{r, fig.cap = "Relationship between 2 first EOFs and amplitude of wave 3. a) Monthly mean wave 3 amplitude as a function of PC1 and PC2; contours are a representation of the linear fit on the right. b) Amplitude of monthly mean wave 3 amplitude as function of the model prediction."}
model <- nls(amplitude ~ a0 + a*sqrt((PC1 - p1_0)^2 + (PC2 - p2_0)^2),
             data = qs3)
cm <- as.list(coef(model))
grid <- setDT(expand.grid(PC1 = seq_range(qs3$PC1, 64),
                          PC2 = seq_range(qs3$PC2, 64)))
grid[, amplitude := predict(model, grid)]
qs3[, pred := predict(model, .SD)]
...
p2 <- ggplot(qs3, aes(pred, amplitude)) +
   geom_point(alpha = 0.5, size = 0.5) +
   geom_abline() +
   # geom_smooth(method = "lm", formula = y ~ x,
   # color = "black", se = T) +
   # geom_line(data = grid) +
   scale_x_continuous(expression(a[0] + a%*%sqrt((PC1 - p[1])^2 + (PC2 - p[2])^2)),
                      limits = c(0, NA)) +
   scale_y_continuous("Wave 3 amplitude") +
   coord_equal() +
   labs(tag = "b") 
...
```{r, fig.cap = "Relationship between wave 3 phase and angle betweenPC2 and PC1."}
ggplot(qs3, aes(phase*180/pi*3, 
                ConvertLongitude(atan2(PC2, PC1)*180/pi))) + 
   geom_point() +
   geom_abline() +
   scale_x_continuous("Phase (degrees)") +
   scale_y_continuous(expression(tan^{-1}~(PC2/PC1)/3)) +
   coord_equal()
```
...
qs3[, .(time, PC1, PC2, phase)] %>% 
   melt(id.vars = c("time", "phase"), variable.name = "PC") %>% 
...
```{r, fig.cap = "Angle between PC1 and PC2 and Real and Imaginary part of wave 3"}

...
qs.eof <- copy(ncep[lev == 200 & lat %between% lats.eof]) %>%
   ...
   .[, gh := Detrend(gh, time),
     by = .(lon, lat)] %>%
   EOF(data = ., gh ~ lon + time |  lat, n = 1:10, B = 5000)
...
binwidth <- 0.0015
psi.regr %>% 
   .[, psi.z := Anomaly(estimate), by = .(lat, term, method, season)] %>% 
   .[, c("f.lon", "f.lat") := WaveFlux(.SD), 
     by = .(term, season, method)] 
ggplot(psi.regr, aes(lon, lat)) +
   geom_contour_fill(aes(z = psi.z*10^-9),
...
ice.eof[, x1 := - 3950*1000 + 25000*x]
ice.eof[, y1 := - 3950*1000 + 25000*y]
...
regr.v <- ice.drift.sem %>% 
   ...
   .[, FitLm(v/1000, PC1, PC2, se = TRUE), 
     by = .(x, y, season)] %>% 
   .[term != "(intercept)"] %>% 
   .[, significant := prob.t(estimate, std.error, df)]
```
...
ice.drift.regr %>% 
   # .[term == "PC1" & season == "MJJASON"] %>%
   .[, significant := prob.t(estimate, std.error, df)] %>% 
   dcast(x + y + season + term ~ variable, 

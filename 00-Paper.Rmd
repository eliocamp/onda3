...
```{r ice-east-west, fig.cap = c("Mean sea ice concentration at the east of 90°W vs. at the west.")}
ice.trim[,
...
ggplot(qs3, aes(sqrt(PC1^2 + PC2^2), amplitude)) +
...
```{r phase-PCs, fig.cap = "Phase of zonal wave 3 and value of PCs 1 to 3. The line is a loess smooth."}
smoothcircular <- function(data, range = c(0, 360), span = 0.75, n = 50) {
   if (!exists("d")) d <<- copy(data)

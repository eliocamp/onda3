...
On the other hand, if it is a mix of different local modes, then we would expect to see a lack of correlation between separated longitudes. 
...
Now, I compute the cross-correlation between all pairs of indices, which is shown in the next figure.
...
* Eurasian sector (between 0º and 145ºE)
* Pacific sector (between 145ºE and 100ºW) 
* Atlantic sector (between 100ºW and 180ºW)

So, for example, local EOFS in the Pacific sector share correlations above 0.8 among themselves, but low (0.2--0.3) correlations with EOFs in the Atlantic sector and no correlation (< 0.2) with EOFs in the Eurasian sector. 

This strongly suggest that there are there relatively independent modes of variability instead of a single hemispheric mode. EOFs in the Eurasian sector are not as highly correlated among themselves as the other two, so they might not form a single robust area of joint variability.  
...
Local EOFs at 50 hPa seem to be well correlated along the whole hemisphere. The minimum correlation is between North America and European local EOFs, but even those are a very respectable 0.4. This suggests a more robust hemispheric mode. 
...
The next figure shows the correlation between geopotential height anomalies at 1000 hPa and these regional EOFs defined as the mean index of the local EOFs belonging to that region. 

```{r plot-field-cor, fig.height = 5, fig.width=9} 
#| fig.cap: Correlation between 1000 hPa geopotential height and the regional EOF index computed as the mean indices of local EOFs belonging to that region (shaded) and NAM (contours).
...
This further suggest the independent nature of these three (or two) different modes of variability that are mixed in the NAM. 

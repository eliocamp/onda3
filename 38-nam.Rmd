...
```{r reof}
reof <- hgt %>% 
   .[, .(eof = list(EOF(hgt_a ~ time | lon + lat, n = 1:10, 
                        data = .SD, rotate = TRUE))),
     by = lev]
...
```{r geom_nam} 
geom_nam <- function(levs = nam$lev, PC = TRUE) {
   data <- nam[, eof[[1]]$right, by = lev] %>% 
      .[lev %in% levs] %>% 
...
This further suggest the independent nature of these three (or two, if one doesn't believe the robustness of the Eurasian EOF) different modes of variability that are mixed within the NAM. 

The relationship between the regional 1000 hPa EOFs and geopotential height at 200 hPa follows a similar story but with smoother patterns. The correlation pattern with the Pacific region EOF shows a more evident wave train at this level, which probably betrays that the PNA is probably responsible of this regional EOF.
...
This suggests that the leading EOF at the lower stratosphere, which is a proper annular mode, is not associated with an annular mode in the troposphere, but with the regional Atlantic and Eurasian modes (remembering that the latter is not as well defined as the former).
...
reof %>% 
   ...
   scale_fill_divergent(NULL, guide = "none") +
   geom_qmap() +
   geom_nam(lev = 1000, PC = FALSE) +
   coord_polar()  +
   facet_wrap(.~PC, labeller = labeller(lev = lev.lab))
```

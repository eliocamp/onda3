...
```{r repr-fig2, fig.cap="Onda 3 mensual de cada simulación y para ERA5."}
datagoyal %>% 
   keep_lat() %>% 
...
mean_amplituide %>%  
   ...
   scale_size_exp() +
   coord_flip() +
   scale_y_continuous("Amplitud media de las ondas zonales") +
   scale_x_latitude()
```
...
```{r amplitud-media-lat, fig.cap = "Amplitud de las ondas zonales medias por latitud."}
amplitude_mean %>%  
   .[lat <= -20] %>% 
   ...
   scale_size_exp() +
   coord_flip() +
   scale_y_continuous("Amplitud de las ondas zonales medias") +
   scale_x_latitude()
```
...
stat <- datagoyal %>% 
   .[, FitWave(hgt, k = 1:3), by = .(lat, exp, time)] %>% 
   .[, c("ma", "mp") := mean.wave(amplitude, phase, k)[c("amplitude", "phase")], 
     by = .(lat, exp, month(time), k)] %>% 
   .[, .(stationarity = mean(cos(k*(mp - phase))*amplitude/ma)), 
     by = .(lat, exp, k)]
...
Para esta medida, la corrida control es increíblemente mala. 
En latitudes medias la estacionariedad es mucho menor que la observada y la distribución general es muy distinta, con un mínimo en 40ºS que coincide con el máximo en las observaciones. 
Qué es lo que pasa en las latitudes polares es un misterio. 

Este problema de la corrida control es muy raro considerando que las corridas experimentales tienen una estructura que sigue la forma de las observaciones aunque la magnitud sea considerablemente menor. 
...
En resumen, superficialmetne 


para la onda 3, la simulación aquaplanet tiene una amplitud ligeramente menor, pero no lo suficiente para explicar el cambio dramático en la amplitud de la onda 3 del campo medio.
...
```{r envelope-medio-1, fig.cap = "Envolvente media de las ondas zonales de cada experimento. En etiqueta, el desvío estándar sobre el valor medio en 55ºS.", fig.height=7, fig.width=6}
envolvente_media <- datagoyal %>% 

...
datagoyal <- datagoyal %>%
   ...
   # copy() %>% 
   .[, ":="(hgt_z = hgt - mean(hgt),
            hgt_sd = sd(hgt)), keyby = .(exp, time, lat)] %>% 
   
   .[, hgt_z := hgt_z/mean(hgt_sd), by = .(lat, exp)] %>% 
   .[, zw3 := FilterWave(hgt, 3), keyby = .(exp, time, lat)] %>% 
   .[, zw3_std := zw3/sd(zw3), keyby = .(exp, time, lat)] %>% 
   .[, qs3 := mean(zw3), by = .(exp, lon, lat, month(time))] %>%
   .[, qs3_std := qs3/sd(qs3), by = .(exp, time, lat, month(time))] %>%
   .[, ":="(cov_zw = periodic_roll_fun(hgt_z*zw3_std, width = w, fun = roll::roll_sum), 
            cov_qs = periodic_roll_fun(hgt_z*qs3_std, width = w, fun = roll::roll_sum)),
     keyby = .(exp, time, lat)]
...
datagoyal %>% 
   ...
   .[time == unique(time)[2]] %>% 
   keep_lat() %>% 
   ggplot(aes(lon, hgt_z)) +
   geom_line(aes(color = "Z*")) +
   geom_line(aes(y = zw3_std, color = "Onda 3")) +
   geom_line(aes(y = cov_zw/max(cov_zw), colour = "Proyección")) +
   scale_x_longitude() +
...
datagoyal %>% 
   ...
   periodic_lon() %>% 
   ggplot(aes(lon, lat)) +
   # geom_contour_fill(aes(z = cov_zw)) +
   geom_contour2(aes(z = zw3, linetype = factor(-sign(..level..))), color = "gray50") +
   geom_contour2(aes(z = hgt_z, linetype = factor(-sign(..level..)), 
                     label = ..level..), family = hrbrthemes::font_rc,
                 label.placer = label_placer_flattest(45)) +
   geom_qmap() +
   geom_coords() +
   coord_polar() +

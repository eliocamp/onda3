...
ggplot(ncep.qs[lat <= 0 & k == 3], aes(lat, lev)) +
   ...
   scale_fill_viridis_c(name = "Amplitud   ",
...
```{r stat, fig.cap = "Mean wave amplitude (shaded) and stationarity (contours).", fig.width=10, fig.height=6}
binwidth.ampl <- 10
binwidth.stat <- 0.1
breaks <- 2^seq(0, log2(650), by = 1)
ggplot(ncep.qs[lat <= 0 & k == 3], aes(lat, lev)) +
   # geom_contour_fill(aes(z = mean.amplitude), breaks = MakeBreaks(10)) + 
   geom_contour_fill(aes(z = asin(amoma)*2/pi), breaks = MakeBreaks(0.1)) +
   # geom_text_contour(aes(z = asin(amoma)*10*2/pi), breaks = c(7, 9), skip = 0, 
   #                   rotate = FALSE, stroke = 0.1, stroke.color = "#2B828C") +
   surface +
   geom_index.region(rect.annotation) +
...
```{r boxplot-index, fig.cap = "Seasonal cicle of QS3 index.", fig.width=190/25.4, fig.height=113/25.4}
set.seed(42)
...
ggplot(gh.g, aes(lon, lat)) +
   ...
   map.world +
   # scale_s_map() +
   scale_x_longitude(ticks = 90) +
   scale_y_latitude(ticks = 60) +
...
```{r fase-boxplot, fig.cap = "Phase of the mean wave for each month and $\\pm$ 1 standrad deviation (black dots and error bars). In red and blue, phase of the most intense 20 events for each month, representing the location of the ridge and throuht respectively.", fig.height=109/25.4, fig.width=226/25.4}
lat.lims <- c(-60, -22)
lon.lims <- c(240, 360)

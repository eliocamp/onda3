...
ncep[lev == 200 & lat <= 0 & month(time) == 1, mean(gh), 
   ...
   .[, .(lon, FilterWave(V1, -1:0)), keyby = .(lat, month)] %>% 
   ggplot(aes(lon, lat)) +
...
```{r gh3-season, fig.cap = "Wave 3 component of the geopotential field of each season at 200hPa.", fig.height=220/24.5, fig.width=210/24.5}
binwidth <- 7
htal <- ggplot(gh.season[lev == 200], aes(lon, lat)) +
...
```{r qs3-season-cut, fig.cap = "Mean wave 3 component of geopotential height between 65°S and 35°S", fig.height=220/24.5, fig.width=210/24.5}
cutlats <- c(-65, -35)
binwidth <- 7
r <- 1/cos(mean(c(-90, -15))*pi/180) 
asp <- 360/75/r

vtcal <- ggplot(gh.season[lat %between% cutlats, .(qs3 = mean(qs3)), 
                 by = .(lon, lev, season)], 
       aes(lon, lev)) +
   geom_contour_fill(aes(z = qs3), breaks = AnchorBreaks(0, binwidth, 0)) +
...
```{r qs3-season, fig.height=(105*2)/24.5, fig.width=207/24.5}
(htal + vtcal + plot_layout(ncol = 1)) * 
   scale_fill_divergent(breaks = AnchorBreaks(0, binwidth, 0), 
                        name = "Altura geopotencial   ",
                        limits = pm(40),
                        guide = guide_colorstrip_bottom()) 
...
```{r ampl, fig.cap = "Mean wave amplitude (shaded) and stationarity (contours).", fig.width=202/24.5, fig.height=115/24.5}
binwidth.ampl <- 10
binwidth.stat <- 0.1
breaks <- 2^seq(0, log2(650), by = 1)
ampl <- ggplot(ncep.qs[lat <= 0 & k == 3], aes(lat, lev)) +
...
```{r stat, fig.cap = "Mean wave amplitude (shaded) and stationarity (contours).", fig.width=202/24.5, fig.height=115/24.5}
binwidth.ampl <- 10
binwidth.stat <- 0.1
breaks <- 2^seq(0, log2(650), by = 1)
stat <- ggplot(ncep.qs[lat <= 0 & k == 3], aes(lat, lev)) +
...
```{r ampl-stat, fig.width=407/24.5, fig.height= 132/24.5}
ampl + stat
```
...
```{r regr-psi-ncep, fig.cap = "Regression of 200hPa geopotential height and QS3 standarized index, and wave acivity flux computed from regression of 0.201 sigma.", fig.height=240/24.5, fig.width=840/24.5}

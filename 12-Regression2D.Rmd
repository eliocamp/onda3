...
```{r}
cv_regression$summary$eof <- sum(cv_regression$coef_eof != 0)
lasso_regression$summary$eof <- sum(lasso_regression$coef_eof != 0)

rbind(cv = unlist(cv_regression$summary[c("r2", "p.value", "eof")]),
      lasso = unlist(lasso_regression$summary[c("r2", "p.value", "eof")]))
```
...
El $r^2$ también tiene un comportamietno similar. 

```{r}
lasso_v_cv("r2")[, lapply(.SD, round, 2)]
```

Comparando el $r^2$ en función de la cantidad de coeficientes no nulos para el método LASSO, crossvalidación y el "ingénuo" se ve que el método de crossvalidación deja de incluir EOFs en el "codo" de la curva, mientras que LASSO los sigue incluyendo. 
...
,
```{r}
point_regression[term == "year", 
                 mean(pt(abs(estimate)/std.error, df, lower.tail = FALSE) <= 0.05)]
```

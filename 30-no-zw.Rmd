...
En lo que sigue, voy a tomar "gajos" de longitud de 120º de ancho (es decir, media longitud de onda) y calcular la correlación entre el campo de altura geopotencial y de onda 3 para cada gajo.
Es decir, correlación y covarianza móvil con una ventana de 120º.
Lo voy a hacer para las tres combinaciones con `r emo::ji("check")` en la tabla anterior.

La correlación da una idea de similitud entre el campo y la onda 3 y además permite calcular la varianza explicada por la onda 3 hemisférica en cada gajo.
Está afectada solamente por la diferencia de fase ente la onda 3 hemisférica y la onda 3 local.
La covarianza es similar a la correlación, pero tiene en cuenta la amplitud de las anomalías de altura geopotencial.
Como la hago sobre la onda 3 normalizada para tener amplitud 1, lo único que afecta la magnitud de la covarianza es la amplitud de la onda 3 geopotencial local.
...
hgt <- hgt %>% 
   .[, wave3_m_norm := wave3_m/sd(wave3_m), by = .(lat, time)] %>% 
   .[, wave3_norm :=  wave3/sd(wave3), by = .(lat, time)] %>% 
   .[, wave3_a_norm := wave3_a/sd(wave3_a), by = .(lat, time)]

lons <- unique(hgt$lon)
w <- sum(lons  %between% (180 + c(-lon_halfwidth, lon_halfwidth)))
variables <- c("m_z" = "QS3 con Z*",
               "a_a" = "ZW3' con Z*'",
               "z_z" = "ZW3 con Z*")



hgt[, `:=`(cor_m_z = periodic_roll_cor(wave3_m, hgt_z, 
                                        width = w),
            cov_m_z = periodic_roll_cov(wave3_m_norm, hgt_z, 
                                        width = w),
            
            cor_a_a = periodic_roll_cor(wave3_a, hgt_za, 
                                        width = w),
            cov_a_a = periodic_roll_cov(wave3_a_norm, hgt_za,
                                        width = w),
            
            cor_z_z = periodic_roll_cor(wave3, hgt_z, 
                                        width = w),
            cov_z_z = periodic_roll_cov(wave3_norm, 
                                        hgt_z, 
                                        width = w),
            hgt_3 = periodic_roll_regr(wave3, 
                                       hgt_z, width = w)),
     by = .(lat, time)] 
...
lon_wave3 <- hgt %>% 
   melt(id.vars = c("time", "lon", "lat"), 
        measure.vars = patterns(cor = "^cor_", cov = "^cov_")) %>% 
...
lon_wave3 %>% 
   .[, mean(cor > 0), by = .(lat, lon, variable)] %>% 
   ggplot(aes(lon,lat)) +
   geom_contour_fill(aes(z = V1, fill = ..level..)) + 
   geom_contour2(aes(z = V1), size = 0.1) +
...
cors <- lon_wave3 %>% 
   .[, widyr::pairwise_cor(.SD, lon, time, cov), 
     by = .(variable, lat)] 
...
cors %>% 
   ...
   geom_contour2(aes(z = correlation^2), na.fill = 1) +

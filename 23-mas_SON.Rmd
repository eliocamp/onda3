...
flux_regr %>% 
   # rm_singleton() %>%
   ggplot(aes(lon, lat)) +
   geom_contour_fill(aes(z = vt_mean)) +
...
ceofs %>% 
   regress(datos[lev == 925], air) %>% 
   plot(bins = 20)
```


```{r}
regress(ceofs, sst, sst) %>% 
   plot(na.fill = TRUE, map.fill = "white") +
   scale_y_latitude(limits = c(-80, 20))
```
...
```{r}
filter_sa(cmap) %>% 
   .[, pp_next := shift2(pp, 1), by = .(lon, lat)] %>% 
   regress(ceofs, ., pp_next) %>%
   plot(keep = 1, skip = NA) +
   facet_nested(PC + term ~ season) +
   annotate("rect", xmin = min(lons), xmax = max(lons), ymin = min(lats), ymax = max(lats),
            fill = NA, color = "black") +
   scale_fill_distiller(palette = "BrBG", direction = 1, limits = pm(10), oob = scales::squish)
```
...
cmap[lat %between% lats & lon %between% lons] %>% 
   ...
   .[unnest_eof(ceofs, ceof, "left"), on = .NATURAL, allow.cartesian = TRUE] %>% 
   sep_ReIm(hgt, longer = FALSE) %>% 
   .[, model := predict(lm(pp ~ R + I, data = .SD[year(time) < 2000]), newdata = .SD), 
     by = .(season, PC)] %>%
   melt(id.vars = c("time", "season", "PC"), measure.vars = c("pp", "model")) %>% 
   na.omit() %>% 
   ggplot(aes(time, value)) +

...
ks_join <- ceofs %>% 
   ...
   .[, tercil := cut(EOF, c(0, 1/3, 2/3, 1), labels = c("low", "medium", "high"))] %>% 
   .[datos, on = .NATURAL, allow.cartesian = TRUE] %>% 
   .[, ksm := suppressWarnings(mean(sqrti(ks))), by = .(lat, lon, lev, season)] %>% 
   .[, ks := suppressWarnings(sqrti(ks))] %>% 
   .[, c(as.list(ggplot2::mean_se(ks)),
         list(ksm = ksm[1], 
              n = .N)), 
     by = .(lat, lon, lev, season, PC, part, tercil)]
```

```{r, fig.cap = "Ks medio para 1 tercil (low) y tercer tercil (high) de PC2"}
ks_join %>% 
   .[lev == 200] %>% 
   .[PC == "PC2"] %>% 
   .[lat <= -10] %>% 
   ...
   scale_x_longitude() +
   scale_y_latitude() +
   scale_fill_divergent(limits = pm(0.003), oob = scales::squish) +
...
ks_join %>% 
   ...
   .[tercil != "medium"] %>% 
   ggplot(aes(lon, lat)) +
   geom_raster(aes(fill = y - ksm)) +
   geom_map2(lat <= 0) +
   geom_vline(xintercept = 210) +
   scale_x_longitude() +
   scale_y_latitude() +
   scale_fill_divergent(limits = pm(0.001), oob = scales::squish) +
   coord_quickmap(ylim = c(-85, -10)) +
   facet_nested(season ~ PC + part + tercil)
```
...
ks_join %>% 
   ...
   geom_line(aes(y = ksm)) +
   geom_ribbon(aes(ymin = ymin, ymax = ymax, fill = tercil), alpha = 0.2) +

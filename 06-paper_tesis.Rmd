...
```{r ampl-ncep, fig.cap = "Stationarity of QS3 (shaded, defined in text) and fourier amplitude of the mean zonal wave 3 (contours)."}
binwidth.ampl <- 10
binwidth.stat <- 0.1
...
```{r, fig.cap = "Stationarity of QS3 (shaded, defined in text) and mean amplitude of the zonal wave 3."}
binwidth.ampl <- 10
binwidth.stat <- 0.1
ggplot(ncep.qs[lat <= 0], aes(lat, lev)) +
   geom_contour_fill(aes(z = fun.mean/mean.fun), breaks = MakeBreaks(binwidth.stat),
                     na.rm = TRUE) + 
   geom_contour2(aes(z = fun.mean/mean.fun), breaks = 0.5, linetype = 2) +
   geom_contour2(aes(z = mean.fun), binwidth = binwidth.ampl) +
   geom_text_contour(aes(z = mean.fun), color = "black", 
                     binwidth = binwidth.ampl,
                     stroke.color = "#2B828C", stroke = 0.15) +
   surface +
   geom_index.region(rect.annotation) +

...
theme_elio <- hrbrthemes::theme_ipsum_rc(base_size = 11) +
...
```{r ampl-ncep, fig.cap = "Fourier amplitude of the mean zonal wave 3 (contours)."}
...
index <- ncep[, qs3.index(gh, lat, lev), by = time]
```
...
```{r fase-boxplot, fig.cap = "Phase of the mean wave for each month and $\\pm$ 1 standrad deviation (black dots and error bars). In red and blue, phase of the most intense 20 events for each month, representing the location of the ridge and throuht respectively.", fig.class = "fullpage"}
lat.lims <- c(-60, -22)
lon.lims <- c(240, 360)
...
map.data.nz <- BuildMap(countries = T) %>% 
   .[lat %between% (lat.lims + c(0, 10)) & 
     long %between% (lon.lims - 120 + c(-10, 0))] %>% 
   .[, long := long + 120] %>% 
   .[, lat := lat]

map.arg <- geom_polygon(data = map.data.arg, aes(long, lat, group = group),
...
index %>% 
   ...
   geom_point(data = index %>%
              ...
              aes(ifelse(phase*180/pi + 240 - 60 < 240, NA,
                         phase*180/pi + 240 - 60)),
              alpha = 0.8, size = size+1, color = "blue",
              shape = 18,
              position = position_nudge(y = nudge)) +
   geom_point(data = index %>% 
                 group_by(month = month(time)) %>% 
   ...
   scale_x_longitude(ticks = 20, name = "Longitude \n (South America)", 
                     sec.axis = sec_axis(~ .-120, name = "Longitude \n (New Zealand)",
                                         labels = LonLabel)) +
   scale_y_continuous(breaks = (-(1:12) + 6)*2.6 - 45, 
                      minor_breaks = NULL,
                      labels = month.abb_sp, 
                      # limits = c(-70, -22),
                      name = "Month", expand = c(0, 0)) +
...
map.data.arg <- BuildMap(countries = T) %>% 
   .[lat %between% c(lat.lims + c(0, 10)) & long %between% lon.lims] %>% 
   .[, lat := scales::rescale(lat, c(12, 1), from = lat.lims)] %>% 
   .[, lon := scales::rescale(long, c(0, 360), from = lon.lims)] %>% 
   .[, lon := as.radians(lon)]
...
map.data.nz <- BuildMap(countries = T) %>%
   .[lat %between% (lat.lims + c(0, 10)) &
     long %between% (lon.lims - 120 + c(-10, 0))] %>%
   .[, lat := scales::rescale(lat, c(12, 1), from = lat.lims)] %>% 
   .[, lon := scales::rescale(long, c(0, 360), from = lon.lims - 120)] %>% 
   .[, lon := as.radians(lon)]

alpha <- 0.2
map.arg <- geom_polygon(data = map.data.arg, aes(lon, lat, group = group),
                        fill = "gray70", color = "white", size = 0.1,
                        inherit.aes = F, alpha = alpha)

map.nz <- geom_polygon(data = map.data.nz, aes(lon, lat, group = group),
                        fill = "gray90", color = "black", size = 0.1,
                        inherit.aes = F, alpha = alpha)
...
index %>% copy() %>% 
   ...
   geom_raster(stat = "density", aes(x = phase, y = month(time), fill = ..count..,
                                     group = month(time)),
               circular = TRUE) +
   map.arg + 
   map.nz +
...
Esta es otra forma de mostrar la variabilidad de la onda 3. Son distribuciones de densidad de la fase pesada por la amplitud para cada mes. Se ven las diferencias de fase media entre meses de verano e invierno y también lo cambios en la estacionarieda de la onda. En noviembre, la distribución es prácticamente uniforme. 


:::


```{r ampl-stat, fig.cap = "Mean wave amplitude (shaded) and stationarity (contours)." }
binwidth.ampl <- 10
binwidth.stat <- 0.1
breaks <- 2^seq(0, log2(650), by = 1)
ggplot(ncep.qs[lat <= 0], aes(lat, lev)) +
   geom_contour_fill(aes(z = mean.amplitude), breaks = breaks) + 
...
```{r fun-similar.field}
similar.field <- function(months = 1:12, n = 1:4) {
   index[month(time) %in% months] %>% 
      # .[greater(amplitude, 10)] %>% 
      .[, .(amplitude/sd(amplitude), phase/sd(phase))] %>%
      as.matrix() %>% 
      dist() ->x
   ...
   similar <- rbindlist(lapply(n, function(i) index[month(time) %in% months][tops[[i]]]), 
                        idcol = TRUE)
   ...
   similar.fields <- ncep[lev == 200 & lat < 0] %>% 
      ...
      .[similar[, .(time, field, .id, dist = round(dist, 3))], 
   ...
   print(
      ggplot(similar.fields, aes(lon, lat)) +
         ...
         scale_s_map() +
         coord_quickmap() +
         facet_grid(dist~field, labeller = labeller(dist = AddPreffix("Distancia = ")))
   )
...
```{r top4, fig.cap = "Four most similar field tuples in standarized wave 3 amplitude and phase space. Temporal anomaly of 200hPa geopotential (contours) and zonal anomaly of 200hPa geopotential with waves 1 and 2 filtered out (shaded)." }
cors <- similar.field()
```

::: notes

Se muestran los cuatro pares de campos más similares en cuanto a amplitud y fase. Aún en esta muestra puede verse grandes heterogeneidades. En el primer par de campos, por ejemplo, uno muestra anomalías zonales significativas principalmente sobre el Atlántico y el Índico mientrs que en el otro éstas ocupan todo el dominio. La latitud de las anomal ías también cambia mucho entre ambos casos. La correlación de campo es de `r round(cors[1], 2)`. El segundo y tercer par de campos presenta mayor similaridad (correlación de campo `r round(cors[2], 2)` y `r round(cors[3], 2)` respectivamente). El cuarto par no sólo presenta poca correlación (`r round(cors[4], 2)`) sino que presenta una gran diferencia en el área de máximas anomalías.

Los campos de anomalías temporales, por su parte, casi no presentan similitudes entre los campos. Esto se debe principalmente a que no se está filtrando las ondas 1 y 2.

:::

```{r top4-sep, fig.cap = "Four most similar september field tuples in standarized wave 3 amplitude and phase space. Temporal anomaly of 200hPa geopotential (contours) and zonal anomaly of 200hPa geopotential with waves 1 and 2 filtered out (shaded)." }
cors <- similar.field(9)
```

```{r percentiles fig.cap = ""}
index[, percentile := Percentile(amplitude), by = month(time)]
index[, next.percentile := shift(percentile, 3, type = "lead")]

breaks <- seq(0, 1, by = .25)
set.seed(42)
ggplot(index, aes(percentile, next.percentile)) +
   geom_jitter(width = 0.005, height = 0.005) +
   geom_smooth(method = "lm") +
   scale_x_continuous("Percentile of month", breaks = breaks,
                      minor_breaks = NULL) +
   scale_y_continuous("Percentile of next month", breaks = breaks,
                      minor_breaks = NULL) +
   coord_equal()
```
...
Se muestra la (falta de) relación entre el percentil de cada mes con el percentil del mes siguiente. 

:::   


```{r}
acf.sig(index$percentile, lag.max = 60) %>% 
   .[-1, ] %>% 
   ggplot(aes(lag, acf)) +
   geom_ribbon(aes(ymin = -sig.cut, ymax =  sig.cut), alpha = 0.1) +
   geom_line() 
```
...
El autocorrelograma confirma esta falta de relación.
...
```{r}
index %>% copy() %>% 
   .[, phase := phase*3*180/pi] %>% 
   ggperiodic::periodic(phase = c(0, 360)) -> index.p


index.p %>% 
   .[, amplitude := as.numeric(cut(Percentile(amplitude), 
                        seq(0, 1, by = 0.25), labels = FALSE,
                        ordered_result = TRUE)), 
     by = month(time)] 
   
ggperiodic::wrap(index.p, phase = c(-180, 540)) %>% 
        ggplot(aes(phase, fill = reorder(as.factor(amplitude), -amplitude))) +
   geom_histogram(breaks = seq_centered(0, 360, length.out = (4*4 + 1))) +
   scale_x_continuous(breaks = no_last(seq(0, 360, length.out = (4*4 + 1)))) +
...
```{r regr-psi-ncep, fig.cap = "Regression of 200hPa geopotential height and QS3 standarized index, and wave acivity flux computed from regression of 0.201 sigma."}

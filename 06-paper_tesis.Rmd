...
x <- as.matrix(x)
top10 <- lapply(1:10, function(i) as.numeric(which(x == ranks[i], arr.ind = TRUE)[1, ]))

similar <- rbindlist(lapply(1:4, function(i) index[top10[[i]]]), idcol = TRUE)
...
cors <- dcast(similar.fields, lon + lat + .id ~ field, value.var = "gh.z") %>% 
   .[, cor(`1`, `2`), by = .(.id)] %>% 
   .$V1

ggplot(similar.fields, aes(lon, lat)) +
   geom_contour_fill(aes(z = gh.z), breaks = AnchorBreaks(0, 20, 0), xwrap = c(0, 360))  +

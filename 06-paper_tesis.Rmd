...
```{r}
m <- as.matrix(index[, .(amplitude/sd(amplitude), phase/sd(phase))])
x <- dist(m) 
...
```{r}
similar <- rbindlist(lapply(c(1, 4), function(i) index[top10[[i]]]))

similar[, o := factor(as.character(time), ordered = TRUE, 
                          levels = as.character(time))]

ncep[lev == 200 & lat < 0] %>% 
   .[, gh.a := Anomaly(gh), by = .(lon, lat, month(time))] %>% 
   .[time %in% similar$time] %>%
   .[similar[, .(time, o)], on = "time"] %>% 
   .[, gh.z := FilterWave(gh, c(-1,- 2, 0)), by = .(lat, time)] %>% 
   ggplot(aes(lon, lat)) +
   scale_fill_divergent() +
   scale_s_map() +
   coord_quickmap() +
   facet_wrap(~o, ncol = 1) -> g

g + geom_contour_fill(aes(z = gh.z))  +
   map.SH + 

g + geom_contour_fill(aes(z = gh.a))  +
   map.SH 

...
cors <- ncep[, qs3 := FilterWave(gh, 3), by = .(lat, lev, time)] %>%
   ...
   .[, .(cor = cor.wave(qs3, qs3.mean)), by = .(lat, lev, time)]
...
cors.season <- ncep[, qs3.mean := mean(qs3), by = .(lon, lat, lev, season(time))] %>% 
   .[, .(cor = cor.wave(qs3, qs3.mean)), by = .(lat, lev, time)] 
cormap <- cors.season[, .(r = mean(cor), r_z = r_z(cor)), by = .(lat, lev, season(time))] 
...
¿Se puede usar a escala mensual como estacionariedad? En bruto, lo que estoy viendo es la correlación de la onda 3 con la onda 3 media de todo el período. Un corrimiento de fase gradual podría verse como un cambio en esta variable aún cuando "localmente" la onda siga siendo relativaemnte estacionaria. 

### Para hacer:

* Fijarse haciendo la correlación del campo tridimensional en vez de promediar tridimensionalmente la correlación por cada nivel y círculo de latitud. 

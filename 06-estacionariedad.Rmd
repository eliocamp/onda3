...
```{r ts, fig.cap = "Stationarity as meassured by AMOMA and AVAR at 45°S and 500hPa. Background rectangles mark El Niño and La Niña events and their intensities."}
S[lat == -45 & lev == 500] %>%
...
La Figura \ref{fig:ts} muestra la serie temporal de estacionariedad según los dos métodos en 45°S y 500hPa con una ventana móvil de 5 años. Es notorio que ambas variables coinciden en gran medida (recordar que un valor mayor de AVAR implica una menor estacionariedad). Una tendencia a la baja con mínimos locales en los 90, mediados de 2010 y máximo a principios de los 2000. 

Una forma alternativa --aunque conceptualmente equivalente-- es la de calcular la medida de distancia para cada mes y luego ajustar un modelo LOWESS usando la amplitud como los pesos. El resultado de este método (Figuras \ref{fig:cor-smooth} y \ref{fig:cor-smooth-corte}) tiene menor varianza en la alta frecuencia pero captura las variaciones temporales. 

```{r cor-smooth, fig.cap = "Correlation between monthly wave and a running mean wave at 45°S and 500hPa with windows of 5 years (gray line), loess fit weigthed by wave amplitude (black line). AMOMA computed with a 5 year running window is shown (in red) as refference.", fig.height=3}
qs[, rho := cos(3*(phase - phi.s))]
qs[, dif := acos(cos(3*(phase - phi.s)))^2]

qs <- qs[!is.na(rho) & lat != -90,
   `:=`(amoma.loess = predict(loess(rho ~ as.numeric(time), weights = amplitude, 
                        span = 0.13*2, data = .SD, degree = 1)), 
        avar.loess = predict(loess(dif ~ as.numeric(time), weights = amplitude, 
                        span = 0.13*2, data = .SD, degree = 1))),
   by = .(lev, lat)]
...
ggplot(qs[!is.na(rho) & lev == 500 & lat == -45], aes(time, rho)) +
   ...
   geom_line(aes(y = amoma.loess)) +
   geom_line(aes(y = value), data = S[lev == 500 & lat == -45 & method == "amoma"], 
             color = "red") +
   # scale_y_continuous(trans = "atanh") +
   # geom_smooth(aes(weight = amplitude), method = "lm") +
...
binwidth <- 0.05
ggplot(qs[lev == 500], aes(lat, time)) +
   
   geom_contour_fill(aes(z = value), data = S[lev == 500 & method == "amoma"], 
                 size = 0.2, breaks = MakeBreaks(binwidth)) +
   geom_contour_tanaka(aes(z = amoma.loess), na.rm = TRUE, 
...
La tendencia hacia menor estacionariedad se da en todos los niveles de latitudes medias (Figura \ref{fig:amoma-trends}) y hay una tendencia a mayor estacionariedad en la estratósfera tropical. 

```{r amoma-trends, fig.cap = "Stationarity for the whole period (contours) and decadal trend in stationarity (shaded) for AMOMA", fig.height=3}
regr.S <- S[, FitLm(value, time = 1:.N, se = TRUE),
     by = .(lat, lev, method)]

ggplot(regr.S[term == "time" & method == "amoma"], aes(lat, lev)) +
   geom_contour_fill(aes(z = estimate*12*10), 
                     breaks = AnchorBreaks(0, 0.02, 0)) +
   # stat_subset(aes(subset = abs(estimate)/std.error > 3), geom = "point", 
               # size = 0.2) +
   geom_contour_tanaka(data = qs[, stationarity.wave2(wave), by = .(lat, lev)],
                       aes(z = V1)) +
   scale_x_latitude(trans = "reverse", ticks = 15) +
   scale_fill_divergent(oob = scales::squish,
                        guide = guide_colorstrip_bottom(),
                        breaks = AnchorBreaks(0, 0.02, 0),
                        limits = pm(0.1)) +
   scale_y_level(breaks = lev.breaks)  +
   annotate("point", x = -28, y = 50)
```
...
```{r}
S <- qs[, .(time, lat, lev, method = "amoma", value = amoma.loess)]
```
...
regr.gh <- S[lat == -45 & lev == 500, .(time, value = Detrend(value), method)] %>% 
   ncep[lev == 500][., on = c("time"), allow.cartesian = TRUE] %>% 
   .[, gh := Anomaly(gh), by = .(lat, lon, month(time))] %>% 
   .[, gh := predict(loess(gh ~ as.numeric(time), span = 0.13*2, data = .SD, 
                           degree = 1)),
     by = .(lat, lon)] %>% 
...
regr.stream <- S[lat == -45 & lev == 500, 
   ...
   .[, psi.a := predict(loess(psi.a ~ as.numeric(time), span = 0.13*2, data = .SD, 
                           degree = 1)),
     by = .(lat, lon)] %>% 
...
regr.stream[, psi.z := estimate] %>% 
   .[, c("f.x", "f.y") := WaveFlux(.SD), by = .(method, term)]
```
...
ggplot(regr.gh[term == "value" & method == "amoma"], aes(lon, lat)) +
   geom_raster(aes(fill = estimate), interpolate = TRUE,
               data = regr.sst[term == "value" & method == "amoma"]) + 
   ...
   geom_streamline(aes(dx = f.x, dy = f.y), min.L = 5, skip = 3,
                   ...
                   data = regr.stream[term == "value" & method == "amoma"]) +

...
```{r ts, fig.cap = "Stationarity as meassured by AMOMA at 45°S and 500hPa. The pre-satellite era is marked in gray."}
...
```{r}
set.seed(42)
wv <- Wavelets(S[lev == 500 & lat == -45 & is.finite(amoma)][, date := time],
      loess.span = 0, "amoma", dt = 1/12, n.sim = 1000, method = "shuffle")
wavelets <- as.data.table(wv)
autoplot(wv, geom = "tanaka") +
   geom_hline(yintercept = c(22, 12), linetype = 3) +
   coord_cartesian(ylim = c(2, max(wavelets$period)))
```

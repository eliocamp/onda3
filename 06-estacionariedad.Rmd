...
ggplot(aes(time, amoma)) +
   geom_rect(xmin = -Inf, xmax = 198, ymin = -Inf, ymax = Inf, 
...
ggplot(qs[!is.na(rho) & lev == 500 & lat == -45], aes(time, rho)) +
   ...
   geom_ribbon(aes(ymin = amoma.loess - 2*ci, ymax = amoma.loess + 2*ci), 
...
ggplot(qs[lev == 500], aes(lat, time)) +
   geom_contour_fill(aes(z = amoma.loess)) +
   geom_contour_tanaka(aes(z = amoma.loess)) +
...
invisible(capture.output(
   WaveletComp::analyze.wavelet(qs[!is.na(rho) & lev == 500 & lat == -45][, date := time],
                                verbose = FALSE,
                             loess.span = 0, "amoma.loess", dt = 1/12) -> wv))
wavelets <- as.data.table(wv)
autoplot(wv) 
```
...
wavelets[, .(power = mean(power), p.value = mean(p.value)),
   ...
   geom_line() +
   geom_vline(xintercept = c(20, 14), linetype = 3) +

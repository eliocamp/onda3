...
max.cor <- function(x, y, lag.max = 10, dl = 1) {
   # lags <- seq(-lagmax, lagmax, by = dl)
   cors <- ccf(x, y, na.action = na.pass, lag.max = lag.max, plot = FALSE)
   # cors <- vapply(lags, function(l) cor(x, shift2(y, l), use = "complete.obs"), 
                     # FUN.VALUE = 1)
   cor <- cors$acf[, 1, 1]
   i <- JumpBy(seq_along(cor), by = dl)
   return(list(cor = cor[i], lag = cors$lag[i, 1, 1]))
}

base <- gh[lat %~% lat0 & lon %~% lon0]$gh1
gh[, max.cor(base, gh1, lag.max = 8, dl = 1), by = .(lon, lat)] -> cormap
cormap[, lag := -lag]

cormap[, approx(lag, cor, xout = seq(-8, 8, by = 0.25)), by = .(lon, lat)] %>% 

ggplot(aes(lon, lat)) + 
   geom_contour_fill(aes(z = y, frame = x, group = x), 
...
gganimate::gganimate(g, interval = 0.1, filename = "share/propagacion.gif")
```


```{r}
cormap[, lag[which.max(cor)], by = .(lon, lat)] %>% 
   .[lat == lat0] %>% 
   ggplot(aes(lon, V1)) +
   geom_line()

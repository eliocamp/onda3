...
gh[, c("R", "I") := ReIm(spectral::analyticFunction(gh1)),
   by = .(time, lat)]
gh[, H := complex(real = R, imaginary = I)]
gh.ceof <- EOF(H ~ time | lon + lat, data = gh, n = 1:6, B = 1000)
```

```{r}
screeplot(gh.ceof, var = "sd") +
...
gh.ceof$right %>% 
ggplot(aes(lon, lat)) +
   geom_raster(aes(fill = Arg(H))) + 
   geom_contour_fill(aes(z = Arg(H))) + 
   geom_contour3(aes(z = Im(H))) +
   # geom_contour(aes(z = abs(H))) +
   map.SH +
   scale_fill_gradientn(colours = c("black", "#4A8A2C" , 
                                    "white", 
                                    "#FFA500", "black")) +
   scale_s_map(ylim = c(-90, -20)) +
   facet_wrap(~PC)  +
...
```{r}
copy(gh.ceof$right)[, H := Re(H)] %>% 
   .[, FitWave(H, k = 2:5), by = .(lat, PC)] %>% 
...
```{r}
cut(gh.ceof, 1:2)$left %>% 
   # .[time %in% unique(time)[1:365]] %>% 
   ggplot(aes(month(time), Mod(H))) +
   geom_boxplot(aes(group = month(time))) 
```

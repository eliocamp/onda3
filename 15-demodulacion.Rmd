...
```{r}
envelopes <- as.data.table(expand.grid(k = 2:6, 
                                       time = unique(SH$time),
                                       hemisphere = unique(hgt$hemisphere)))
lons <- unique(SH$lon)
envelopes <- envelopes[, .(lon = lons, 
                           env = WaveEnvelope(hgt[time == .BY$time & 
                                                  hemisphere == .BY$hemisphere, hgt], 
                                              seq(k-1, k+1))),
                       by = .(k, time, hemisphere)]
```


```{r}
envelopes[, .(mean_env = mean(env)), by = .(k, hemisphere, lon)] %>% 
...
cors_env <- lapply(lons[lons < 180], function(l) {
   envelopes[, .(cor_env = cor(env[lon == l], 
...
```{r}
wavelets[envelopes, on = c("time", "hemisphere", "lon", "k")] %>% 

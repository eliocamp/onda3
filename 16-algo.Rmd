...
Estas nuevas componentes princpales son parecidas a las anteriorer sólo que se agrupan de forma más explícita según la covariabilidad entre 200hPa y 50hPa. En general, tienen una esctructura barotrópica equivalente. En DJF
...
ceof[, ceof[[1]]$left, by = .(dataset, season)] %>% 
   # .[dataset == "era20"] %>% 
   sep_ReIm(hgt) %>% 
   ggplot(aes(year, hgt, color = dataset)) +
   geom_line() +
   # geom_smooth() +
   facet_grid(cPC + part ~ season )
```

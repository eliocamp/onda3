...
```{r, fig.cap = "Detalle de la figura anterior para las estaciones  SON en coordenadas polares.", fig.width=9, fig.height=11}
gdata <- gdata %>%    
   .[season %in% c("SON")] %>% 
   ggperiodic::periodic(lon = c(0, 360))
...
```{r, fig.cap = "Relación entre altura geopotencial y parte Real (sombreado) e Imaginaria (contornos) de cada componente principal. a) correlación, b) regresión.", fig.with = 5, fig.height=9}
...
```{r, fig.cap = "Correlación entre el campo de geopotencial y la parte real e imaginaria de las series temporales de cada EOF. Para ERA20."}
ceof[dataset == "era20"] %>% 
   .[season == "SON"] %>% 
...
ceof[dataset == "era20"] %>% 
   ...
   na.omit()  %>% 
   .[, .(R = cor(R, hgt), 
         I = cor(I, hgt)), by = .(cPC, year, lev)] -> ceof_cor

ceof_cor %>% 
   melt(id.vars = c("cPC", "year", "lev"), variable.name = "part", value.name = "hgt") %>% 
   ggplot(aes(year, hgt)) +
   geom_line(aes(color = part)) +
   geom_smooth(aes(color = part), method = "lm") +
   facet_grid(lev ~ cPC, scales = "free")
```

```{r, fig.cap = "\"Magnitud\" de la correlación de la figura anterior, calculada como la raiz cuadrada de la suma del cuadrado de las correlaciones."}
ceof_cor %>% 
   ggplot(aes(year, Mag(I, R))) +
   geom_line() +
   geom_smooth() +
   facet_grid(lev ~ cPC, scales = "free")
```

...
map <- function(subset, color = "black", size = 0.2, ...) {
   data <- fortify(map_simple())
   subset <- eval(substitute(subset), envir = data)
   
   geom_path(data = data[subset, ], 
...
```{r}
lats.eof <- c(-80, -20)
ceof <- datos[lev %in% c(10, 50, 200) & lat %between% lats.eof] %>%
   .[dataset != "ncep"] %>% 
...
```{r}
eof <- datos[lev %in% c(10, 50, 200) & lat %between% lats.eof] %>%
   .[dataset != "ncep"] %>% 
   .[year %between% common_range(year, dataset)] %>% 
   .[, hgt := Anomaly(hgt),
     by  = .(lat, season, year, lev, dataset)] %>%
   .[, hgt := Anomaly(hgt), by = .(lon, lat, season, dataset, lev)] %>% 
   .[, hgt := hgt/sd(hgt)*sqrt(cos(lat*pi/180)), by = .(dataset, season, lev)] %>% 
   .[, list(eof = list(EOF(hgt ~ year | lon + lat + lev, n = 1:4))), by = .(dataset, season)]
```



```{r}
ceof[, ceof[[1]]$right, by = .(dataset, season, lev)] %>% 
   # .[lev == 50] %>% 
   # .[season == "DJF"] %>% 
   .[dataset == "era"] %>%
   # .[lev == 50] %>% 
...
ceof <- datos[lev %in% c(10, 50, 200) & lat %between% lats.eof] %>%
   ...
   .[, hgt := Anomaly(hgt),
     by  = .(lat, season, year, lev, dataset)] %>%
   .[, hgt := Anomaly(hgt), by = .(lon, lat, season, lev, dataset)] %>% 
   .[, hgt := hgt*sqrt(cos(lat*pi/180))*exp(hgt/(2*H)), by = .(season, dataset, lev)] %>%
   .[, hgt.cpx := spectral::analyticFunction(hgt),
     by = .(lat, season, year, dataset, lev)] %>% 
   .[, hgt := hgt.cpx] %>%
   .[, list(ceof = list(EOF(hgt ~ year | lon + lat + lev, n = 1:2, suffix = "cPC", B = 5000))),
     by = .(dataset, season)]
```


```{r}
ceof[, ceof[[1]]$sdev, by = .(dataset, season)] %>% 
   ggplot(aes(cPC, sd)) +
   geom_col() +
...
ceof[, ceof[[1]]$right, by = .(dataset, season)] %>% 
   ...
   .[dataset == "era"] %>%
   # .[lev == 50] %>% 
   .[, mean(abs(hgt)^2), by = .(dataset, season, lev, cPC)] %>% 
   ggplot(aes(cPC, V1)) +
   geom_col(aes(fill = factor(lev)), position = "dodge2") +
   facet_grid(dataset  ~season)
```
...
ceof[, ceof[[1]]$left, by = .(dataset, season)] %>% 
   .[dataset == "era20"] %>%
   ggplot(aes(year, Im(hgt))) +
   geom_line() +
   geom_smooth() +
   facet_grid(cPC ~ season + dataset)
...
ceof[, ceof[[1]]$right, by = .(dataset, season)] %>% 
   .[dataset == "era"] %>% 
   .[, FitWave(Im(hgt), 1:6), by = .(lat, lev, dataset, cPC, season)] %>% 
...
```{r}
ceof_era[, ceof[[1]]$left, by = .(dataset, season, lev)] %>%
   copy() %>% 
   setnames(hgt = "EOF") %>% 
   datos[., on = c("dataset", "season", "lev", "year")] %>% 
   .[dataset == "era"]  %>% 
   .[, c("R", "I") := ReIm(EOF)] %>% 
   tidyr::pivot_longer(R:I, values_to = "value", names_to = "variable") %>% 
   as.data.table() %>% 
   .[, cor.test(hgt, value, use = "complete.obs")[c("estimate", "p.value")],  
     by = .(lon, lat, lev, season, dataset, cPC, variable)] %>% 
   ...
   map(lat < 0) +
   scale_fill_divergent() +
   facet_nested(lev + cPC  ~  season + variable, 
                labeller = labeller(lev = AddSuffix(" hPa"))) +
   coord_quickmap()
```
...
```{r}
sst <- ReadNetCDF(here::here("DATA/ERA-Interim/sst.mon.mean.nc"),
                  subset = list(latitude = -90:20)) %>% 
   setnames(longitude = "lon", latitude = "lat") %>% 
   .[, lapply(.SD, mean), by = .(lon, lat, year(time), season(time))]

air <- ReadNetCDF(here::here("DATA/ERA-Interim/air.mon.mean.nc"),
                  subset = list(latitude = -90:20,
                                level = list(850, 50))) %>% 
   setnames(longitude = "lon", latitude = "lat", level = "lev") %>% 
...
```{r}
sst[, sst.z := Anomaly(sst, na.rm = TRUE), by = .(year, season, lat)]


ceof_era[, ceof[[1]]$left, by = .(dataset, season, lev)] %>%
   copy() %>% 
   .[, c("Real", "Imaginary") := ReIm(hgt)] %>% 
   melt(id.vars = c("dataset", "season", "lev", "year", "cPC"), measure.vars = c("Real", "Imaginary")) %>% 
   sst[., on = c("season", "year"), allow.cartesian = TRUE] %>% 
   na.omit() %>% 
   .[, cor.test(sst.z, value, use = "complete.obs")[c("estimate", "p.value")],  
     by = .(lon, lat, season, lev, dataset, cPC, variable)] %>% 
   .[, p.val := p.adjust(p.value, "fdr"), by = .(lev, season, cPC, variable)] %>% 
   .[lat > -75] %>%
   ggplot(aes(lon, lat)) +
...
La correlación entre el la anomalía zonal de SST y las series temporales de casa EOF no da nada significativo 
...
ceof_era[, ceof[[1]]$left, by = .(dataset, season, lev)] %>%
   ...
   .[, c("Real", "Imaginary") := ReIm(hgt)] %>% 
   melt(id.vars = c("dataset", "season", "lev", "year", "cPC"), measure.vars = c("Real", "Imaginary")) %>% 
   air850[., on = c("season", "year"), allow.cartesian = TRUE] %>% 
   na.omit() %>% 
   .[, cor.test(t, value, use = "complete.obs")[c("estimate", "p.value")],  
     by = .(lon, lat, season, lev, dataset, cPC, variable)] %>% 
   .[, p.val := p.adjust(p.value, "fdr"), by = .(lev, season, cPC, variable)] %>% 
   .[lat > -75] %>%
   ggplot(aes(lon, lat)) +
...
ceof_era[, ceof[[1]]$left, by = .(dataset, season, lev)] %>% 
   .[, as.list(ccf(Re(hgt), Im(hgt), plot = FALSE)), by = .(season, lev, cPC)] %>% 
   ggplot(aes(lag, acf)) +
   geom_line() +
   geom_point() +
   facet_nested(lev + cPC  ~  season, 

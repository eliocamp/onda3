...
Esto motiva hacer el EOF con ambos niveles juntos de manera de obtener modos de oscilación conjuntos. Lo bueno de esto es que reduce la dimensión del problema ya que se reducen las series temporales a la mitad (porque hay una sola serie temporal para cada componente principal para los dos niveles). Además, voy a estandarizar la variable en función del desvío estándard de cada nivel. 
...
```{r, fig.cap = "Detalle de la figura anterior para las estaciones JJA y SON en coordenadas polares.", fig.width=9, fig.height=11}
gdata <- gdata %>%    
   .[season %in% c("JJA", "SON")] %>% 
   ggperiodic::periodic(lon = c(0, 360))
   # ggperiodic::qwrap(lon = c(0, 360) ~ c(-180, 180))
   # identity()
...
```{r, fig.cap = "Correlación entre altura geopotencial y parte Real (sombreado) e Imaginaria (contornos) de cada componente principal", fig.width=9, fig.height=11}
eof <- ceof_era[, ceof[[1]]]
...
```{r, fig.cap = "Correlación entre altura geopotencial y el valor absoluto del EOF complejo.", fig.width=9, fig.height=11}
ceof_era[, denormalize(ceof[[1]], "left"), by = .(dataset, season)] %>% 
   setnames(hgt = "EOF") %>% 
   datos[., on = c("dataset", "season", "year")] %>%
   .[lev %in% c(50, 200)] %>% 
...
ceof[dataset == "era"] %>% 
   ...
   geom_vector(aes(dx = f.lon, dy = f.lat), skip = 4, min.mag = 1e-6) +
   scale_mag() + 
   map(lat < 10) +
   scale_fill_divergent() +
...
```{r, fig.cap = "Detalle de la figura anterior sólo en SON y la segunda componente principal"}
ceof_era[, ceof[[1]]$left, by = .(dataset, season)] %>%
   sep_ReIm(hgt) %>% 
   sst[., on = c("season", "year"), allow.cartesian = TRUE] %>% 
   na.omit() %>% 
   ...
   ggplot(aes(lon, lat)) +
   geom_contour_fill(aes(z = estimate), na.fill = TRUE) +
   stat_subset(aes(subset = p.val < 0.05 & is.cross(lon, lat, 1)), geom = "point", size = 0.1) +
   # scale_fill_divergent(limits = c(-1, 1)) +
   scale_fill_divergent() +
   facet_nested( cPC + season ~ part,
                 labeller = labeller(lev = AddSuffix(" hPa"))) +
   map(lat < 25, fill = "white") +
   coord_quickmap()
...
air_reg[lev == 50] %>% 
   ...
   # scale_fill_divergent(limits = c(-1, 1)) +
   scale_fill_divergent() +
   facet_nested(cPC + part ~  season + lev,
                labeller = labeller(lev = AddSuffix(" hPa"))) +
   map(lat < 25) +
   coord_quickmap()
```
...
air_reg[lev == 850] %>% 
   ...
   # geom_raster(aes(fill = estimate)) +
   geom_contour_fill(aes(z = estimate), na.fill = TRUE) +
   stat_subset(aes(subset = p.val < 0.01 & is.cross(lon, lat, 2)), geom = "point", size = 0.01) +
   # scale_fill_divergent(limits = c(-1, 1)) +
   scale_fill_divergent() +

...
ensos$left |> 
   ...
   geom_histogram(aes(Arg(eof)), boundary = 0, binwidth = 10*pi/180,
                  inherit.aes = FALSE) +
   geom_point(aes(y = Mod(eof)*1500, color = is_enso_oni(enso34))) +
   ...
   scale_y_continuous("Cuenta (barras)",
                      sec.axis = sec_axis(~.x/1500, name = "Mod(eof) (puntos)")) +
   scale_color_enso()
```
...
enso_son <- ensos$left |> 
   D(season(time) == "SON") |> 
   D(, days := lubridate::days_in_month(time)) |> 
   D(, .(eof = weighted.mean(eof, w = days)), by = .(time = seasonally(time)))
...
rotations_cEOF2 <- lapply(seq(-pi, pi, by = .5*pi/180), function(a) {
   with_enso |> 
      D(, hgt2 := rotate(hgt, a)) |> 
      D(, partial_cor(Re(eof), Re(hgt2), Im(hgt2))) |>

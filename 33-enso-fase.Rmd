...
```{r, fig.cap = "Mapas de regresión de PSA en sus distintas fase con sst (izquierda) y funcíon corriente en 200hPa (derecha)."}
...
enso <- rsoi::download_oni(use_cache = TRUE) %>% 
   as.data.table() %>% 
   .[, .(enso34 = dSST3.4, time = lubridate::as_datetime(Date))] %>% 
   .[time %in% unique(sst$time)]

```

Lo que voy a hacer es calcular el EOF complejo de la SST promediada entre 5ºS y 5ºN.
...
Otra cosa que voy a hacer es rotar el EOF de manera que la parte real tenga la máxima correlación parcial con el índice ENSO 3.4.

```{r ensos}
ensos <- sst %>% 
   .[lat %between% c(-5, 5)] %>% 
   .[, mean(t), by = .(time, lon)] %>% 
   .[, V1 := approx(lon, V1, xout = lon, rule = 2)$y, by = .(time)] %>% 
   .[, V1 := V1 - mean(V1), by = .(month(time), lon)] %>% 
   .[, eof := spectral::analyticFunction(V1), by = time] %>% 
   EOF(eof ~ time | lon, data = ., n = 1)
```
...
Una imagen más intuitiva de este patrón es la Figura \@ref(enso-sst).
...
* los desvíos significativos se dan únicamente en la dirección del centro de anomalías positivas corrido hacia el oeste.
...
hgt <- ReadNetCDF("DATA/ERA-Interim/erai.mon.mean.nc", vars = c(hgt = "z"), 
                  subset = list(level = 200)) |> 
   normalise_coords() |> 
   D(, hgt := hgt/9.8) |> 
   D(, hgt := hgt - mean(hgt), by = .(lon, lat, month(time))) |> 
   D(ensos$left, on = "time") |> 
   na.omit() |> 
   sep_ReIm(eof, longer = FALSE) |> 
   D(, FitLm(hgt, R, I), by = .(lon, lat, season(time))) |> 
   rm_intercept() |> 
   dcast(lon + lat + season ~ term, value.var = "estimate") |> 
   D(, variable := "hgt")

regr <- rbind(regr, hgt, use.names = TRUE)

```


```{r make-slider-images}
# a <- 45*pi/180
sf::sf_use_s2(FALSE)

angles <- seq(-180, 180, by = 10)
dir.create("ensos")
...
breaks_hgt <- AnchorBreaks(exclude = 0, bins = 20)(ranges[variable == "hgt"]$V1*sqrt(2))

h <- ensos$left |> 
   copy() |> 
   D(season(time) == "SON") |> 
   # D(, eof := rotate(eof, angles[a]*pi/180)) |> 
   D(, a := ConvertLongitude(abs(Arg(eof))*180/pi, from = 180)) |> 
   D(, hist(a, plot = FALSE, breaks = seq(0, 360, by = 10)))


h <- data.table(lon_min = h$breaks, 
                lon_max = h$breaks + 10,
                count = h$counts)

h[, prop := count/sum(count)]
...
sink <- lapply(seq_along(angles), function(a) {
   ...
   prop <- ensos$left |> 
      ...
      D(season(time) == "SON") |> 
      D(, eof := rotate(eof, angles[a]*pi/180)) |> 
      D(, mean(acos(cos(Arg(eof))) < 5*pi/180))
   ...
   plot <- regr |> 
      ...
      geom_contour2(data = ~.x[variable == "hgt"], breaks = breaks_hgt,
                    aes(z = R, label = ..level../100, linetype = factor(-sign(..level..))), 
                    family = hrbrthemes::font_rc,
                    label.placer = label_placer_flattest(45)) +
      geom_rect(data = h, 
                alpha = 0.5,
                aes(xmin = lon_min, xmax = lon_max, ymin = -90, 
                    ymax = scales::rescale(count, to = c(-90, -50))), inherit.aes = FALSE) +
      
      annotate("segment", x = ConvertLongitude(abs(angles[a]+5)), y = -90, 
               xend = ConvertLongitude(abs(angles[a]+5)), yend = -50, 
               color = "#c6262e") +
      annotate("text", x = ConvertLongitude(abs(angles[a]+5)), y = -90, color = "#c6262e", 
               family = hrbrthemes::font_rc_light,
               label = scales::percent(h[lon_min == ConvertLongitude(abs(angles[a]))]$prop)) +
...
(ref:cap-slide) Regresión trimestral de SST (sombreado) y altura geopotencial en 200 hPa (contornos). 
...
<input style="width: 50%;" id="valR" type="range" min="-180" max="180" value="0" step="10" oninput="showVal(this.value)" onchange="showVal(this.value)" /> <br/>

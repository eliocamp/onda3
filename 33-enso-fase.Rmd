...
psi <- ReadNetCDF("DATA/ERA-Interim/erai.mon.mean.nc", vars = "vo", 
   ...
   D(, psi.z := Anomaly(psi), by = .(time, lev, lat))
```
...
regr <- 
   rbind(
      ...
      psi |> 
         D(, psi_a := psi.z - mean(psi.z), by = .(lon, lat, month(time))) |> 
...
h <- ensos$left |> 
   ...
   D(, a := Arg(eof)*180/pi) |> 
   D(, hist(a, plot = FALSE, breaks = seq(-180, 180, by = 10))[c("mids", "counts")], by = season(time)) |> 

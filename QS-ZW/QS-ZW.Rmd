...
Zonal waves (ZW; sometimes also called planetary waves) are waves observed in each individual "instantaneous" field. Quasi-stationary waves (QS), on the other hand, are the resulting waves in the mean field. Of course, these definitions depend on which are the "instantaneous field" in question and the averaging timescales used. However, they illustrate that, given a set of fields, ZWs are properties of the *elements* of the set, while the QS is a property of the set as a whole. This is an important distinction with theorical and methodological implications but is not always appreciated in the literature.
...
It can be shown that $S = 1$ for completely stationary waves and that $\lim_{n \rightarrow \inf} S = 0$ for completely non-stationary waves with convergence of $O\left (n^{1/2}\right)$ (where $n$ is the sample size).
...
where $A_i$, $alpha_i$ are their amplitudes and phases and $\phi$ is longitude. The amplitude of the sum of both waves is 

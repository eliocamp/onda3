...
sam_sep <- annular[, eof[[1]]$right, by = .(dataset, lev, hemisphere)] %>% 
   setnames("hgt", "full") %>% 
   .[, c("sym", "asym") := list(mean(full), Anomaly(full)), by = .(lat, lev, dataset, hemisphere)]
...
indexes <- annular[, eof[[1]]$right, by = .(dataset, lev, hemisphere)] %>% 
   ...
   # .[lev %in% unique(c(levs, extremes$lev))] %>% 
   hgt[., on = .NATURAL] 
...
full <- indexes[, FitLm(hgt_a, full = EOF, weights = cos(lat*pi/180), r2 = TRUE),
                by = .(dataset, lev, time, hemisphere)] %>% 
   rm_intercept()
...
indexes <- rbind(full, sep) %>% 
   .[, estimate_norm := estimate/sd(estimate[term == "full"]), by = .(hemisphere, lev, dataset)] %>% 
...
indexes %>% 
   .[dataset == "ERA5"] %>%
   .[as.numeric(hemisphere) == 2] %>% 
   .[, widyr::pairwise_cor(.SD, lev, time, estimate), by = .(dataset, hemisphere, term)] %>% 
   ggplot(aes(item1, item2)) +
   ...
   scale_y_level() +
   coord_equal() +
   facet_nested(dataset + hemisphere~term, labeller = labeller(term = lab_sam))
```



```{r, fig.cap = "Igual a la figura anterior pero para el hemisferio norte."}
indexes %>% 
   .[dataset == "ERA5"] %>%
   .[as.numeric(hemisphere) == 1] %>% 
   .[, widyr::pairwise_cor(.SD, lev, time, estimate), by = .(dataset, hemisphere, term)] %>% 
...
indexes %>% 
   .[, .(estimate = mean(abs(estimate_norm))), by = .(dataset, lev, hemisphere, term, month(time))] %>% 
   periodic(month = 1:13) %>% 
   ...
   facet_grid(term ~ hemisphere, labeller = labeller(term = lab_sam)) 
```


```{r, fig.cap = "Promedio mensual de abs(sym) / (abs(sym) + abs(asym))."}
indexes_wide <- dcast(indexes, dataset + lev + time + hemisphere ~ term, value.var = "estimate_norm")


indexes_wide %>% 
   copy() %>% 
   .[, mean(abs(sym)/(abs(sym) + abs(asym))), by = .(dataset, lev, hemisphere, month(time))] %>% 
   periodic(month = 1:13) %>% 
   ggplot(aes(month, lev)) +
...
vertical_sym <- indexes %>% 
   ...
   .[, FitLm(full, asym, sym, se = TRUE), by = .(dataset, lev, hemisphere)] %>% 
   rm_intercept() %>% 
   .[term == "sym"] 
...
extremes_sym <- vertical_sym %>% 
   .[, rbind(.SD[which.min(estimate)], 
             ...
             .SD[lev <= 200][which.max(estimate)],
             .SD[lev == 700]), by = .(dataset, hemisphere)] %>% 
   unique()
...
indexes %>% 
   .[extremes_sym[, .(dataset, lev, hemisphere)], on = .NATURAL] %>%
   # .[lev %in% extremes_sym$lev] %>%
   .[dataset == "ERA5"] %>% 
   .[, estimate := estimate/sd(estimate), by = .(dataset, lev, hemisphere, term)] %>% 
   dcast(dataset + time + hemisphere + lev ~ term, value.var = "estimate") %>% 
   tidyr::pivot_longer(asym:sym) %>% 
   ...
   facet_nested(lev ~ hemisphere, scales = "free")
```
...
sam_sep %>% 
   .[, .(sym = var(sym*cos(lat*pi/180))/var(full*cos(lat*pi/180))),
     by = .(dataset, lev, hemisphere)] %>% 
   # .[lev %in% levs] %>% 
   melt(id.vars = c("dataset", "lev", "hemisphere"), variable.name = "term", value.name = "estimate2")  %>% 
...
indexes_2d <- hgt[extremes, on = .NATURAL] %>% 
   ...
   .[, rbind(as.data.table(FitLm(hgt_a, sym, asym)),
     ...
     by = .(lon, lat, dataset, hemisphere, lev)] %>% 
   rm_intercept() %>% 
   periodic(lon = c(0, 360)) %>% 
   .[, term := factor(term, levels = names(lab_sam), ordered = TRUE)] 
...
index_wide <- indexes %>% 
   dcast(dataset + time + lev + hemisphere ~ term, value.var = "estimate_norm")

OLR() %>% 
   ReadNetCDF(subset = list(lat = -90:10)) %>% 
   na.omit() %>% 
   .[, olr_a := Anomaly(olr), by = .(lon, lat, month(time))] %>% 
   .[index_wide[as.numeric(hemisphere) == 2 & lev %in% c(50, 700) & dataset == "ERA5"], 
...
```{r}
pcor %>% 
   .[, mean(estimate^2), by = .(dataset, lev, term, month(time), hemisphere)] %>% 
   ggplot(aes(lev, month)) +
   geom_contour_fill(aes(z = V1)) +
   scale_x_level() +
   coord_flip() +
   facet_grid(term ~ hemisphere)
```

```{r}
indexes %>%
   copy() %>% 
   .[, denom := sum(partial_correlation^2), by = .(dataset, lev, time, hemisphere)] %>% 
   .[, mean(partial_correlation^2/denom), by = .(dataset, lev, term, hemisphere)] %>%
   ggplot(aes(lev, V1)) +
   geom_line(aes(color = term)) +
   scale_x_level() +
...
indexes %>% 
   # .[term == "sym"] %>% 
   .[, mean(partial_correlation^2), by = .(dataset, lev, month(time), term, hemisphere)] %>% 
   ggplot(aes(month, lev)) +
   geom_contour_fill(aes(z = V1)) +
   scale_y_level() +
   scale_fill_viridis_c() +
...
indexes %>% 
   .[term == "sym"] %>% 
   .[, mean(partial_correlation^2/r.squared), by = .(dataset, lev, month(time), term, hemisphere)] %>% 
   ggplot(aes(month, lev)) +
   geom_contour_fill(aes(z = V1)) +
   scale_y_level() +
...
```{r}
dcast(pcor, dataset + lev + time + hemisphere ~ term, value.var = "estimate") %>% 
   .[, cor(asym^2, sym^2), by = .(dataset, lev, hemisphere)] %>% 
   ggplot(aes(lev, V1)) +
